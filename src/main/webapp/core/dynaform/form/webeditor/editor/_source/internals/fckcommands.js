...
function getURLParams(){
	...
	var application = parent.document.getElementById('application').value;
	var formid = parent.document.forms[0].elements['content.id'].value;
	var URLParams="?action=new&application="+application+"&moduleid="+moduleid+"&formid="+formid;
	/*if (isCheckboxFieldSelected()){
		URLParams = 'dialog/checkboxfield.jsp?action=modify';
		URLParams = URLParams + '&moduleid=' + moduleid + '&application=' + application;
	
	}
	else {
		URLParams = 'dialog/checkboxfield.jsp?action=new';
		URLParams = URLParams + '&moduleid=' + moduleid + '&application=' + application;
	}*/ 
...
FCKCommands.GetCommand = function( commandName )
{
	...
	switch ( commandName )
	{
		...
		case 'TableProp'	: oCommand = new FCKDialogCommand( 'Table'		, FCKLang.DlgTableTitle			, 'dialog/fck_table.html?Parent', 480, 300 ) ; break ;
		case 'TableCellProp': oCommand = new FCKDialogCommand( 'TableCell'	, FCKLang.DlgCellTitle			, 'dialog/fck_tablecell.html'	, 550, 350 ) ; break ;
		...
		case 'Form'			: oCommand = new FCKDialogCommand( 'Form'		, FCKLang.Form			, 'dialog/fck_form.html'		, 420, 350 ) ; break ;
		case 'Checkbox'		: oCommand = new FCKDialogCommand( 'Checkbox'	, FCKLang.Checkbox		, 'dialog/myapps_checkboxfield.jsp'+getURLParams()	, 600, 400 ) ; break ;
		case 'Radio'		: oCommand = new FCKDialogCommand( 'Radio'		, FCKLang.RadioButton	, 'dialog/myapps_radiofield.jsp'+getURLParams()	, 600, 400 ) ; break ;
		case 'TextField'	: oCommand = new FCKDialogCommand( 'TextField'	, FCKLang.TextField		, 'dialog/myapps_inputfield.jsp'+getURLParams()	, 600, 400 ) ; break ;
		case 'Textarea'		: oCommand = new FCKDialogCommand( 'Textarea'	, FCKLang.Textarea		, 'dialog/myapps_textareafield.jsp'+getURLParams()	, 600, 400 ) ; break ;
		case 'HiddenField'	: oCommand = new FCKDialogCommand( 'HiddenField', FCKLang.HiddenField	, 'dialog/fck_hiddenfield.html'	, 420, 350 ) ; break ;
		case 'Button'		: oCommand = new FCKDialogCommand( 'Button'		, FCKLang.Button		, 'dialog/myapps_buttonfield.jsp'+getURLParams()		, 600, 400 ) ; break ;
		case 'Select'		: oCommand = new FCKDialogCommand( 'Select'		, FCKLang.SelectionField, 'dialog/myapps_selectfield.jsp'+getURLParams()		, 600, 400 ) ; break ;
