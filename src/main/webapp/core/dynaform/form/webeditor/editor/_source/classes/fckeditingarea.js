...
function examineProjectName (){
	if(frames["framesObj"].document != null){
		var framesObj = frames["framesObj"].document.body;
		var imgs = framesObj.getElementsByTagName("img");
		var imgLength = imgs.length;
		var table = framesObj.getElementsByTagName("table");
		var tableLength = table.length;
		if(tableLength != 0){
			 for (var a = 0; a  < tableLength; a++) {
		         var bordercolor = table[a].getAttribute("bordercolor");
		         var tableClass = table[a].className;
		         var tableClass1 = '"' + tableClass + '"';
		         if (bordercolor == "#b4ccee"){
		        	 table[a].removeAttribute("bordercolor");
		        	 var tableBorder = table[a].getAttribute('border');
		        	 if(tableBorder != 0){
		        		 table[a].style.border = tableBorder + 'px solid #b4ccee';
		        	 }
		        	 var td = table[a].getElementsByTagName("td");
		        	 var tdLength = td.length;
		        	 if(tdLength != 0){
		        		 for (var u = 0; u  < tdLength; u++) {
		        			 if(tableBorder != 0){
					        	 td[u].style.border = '1px solid #b4ccee';
		        			 }
		        		 }
		        	 }
		         }
		         if(tableClass1.indexOf(' fckTable ') != -1 || tableClass1.indexOf('"fckTable ') != -1 || tableClass1.indexOf(' fckTable"') != -1 || tableClass1 == '"fckTable"'){
			         if(tableClass1.indexOf(' fckTable ') != -1){
			        	 table[a].className = tableClass.replace("fckTable ","");
			         }else if(tableClass1.indexOf('"fckTable ') != -1){
			        	 table[a].className = tableClass.replace("fckTable ","");
			         }else if(tableClass1.indexOf(' fckTable"') != -1){
			        	 table[a].className = tableClass.replace(" fckTable","");
			         }else if(tableClass1 == '"fckTable"'){
			        	 table[a].className = "";
			         }
		        	 var tableBorder = table[a].getAttribute('border');
		        	 if(tableBorder != 0){
		        	 ...
		        	 if(tdLength != 0){
		        		 for (var u = 0; u  < tdLength; u++) {
		    		         var tdClass = td[u].className;
		    		         var tdClass1 = '"' + tdClass + '"';
		    		         if(tdClass1.indexOf(' fckTd ') != -1){
					        	 td[u].className = tableClass.replace("fckTd ","");
					         }else if(tdClass1.indexOf('"fckTd ') != -1){
					        	 td[u].className = tableClass.replace("fckTd ","");
					         }else if(tdClass1.indexOf(' fckTd"') != -1){
					        	 td[u].className = tableClass.replace(" fckTd","");
					         }else if(tdClass1 == '"fckTd"'){
					        	 td[u].className = "";
					         }
		        			 if(tableBorder != 0){
					        	 td[u].style.border = '1px solid #b4ccee';
		...
		if(imgLength != 0){
			 for (var i = 0; i  < imgLength; i++) {
		         var txturlPath = imgs[i].getAttribute("txturlPath");
		         if (txturlPath == "txturlPath") {
		        	 var absolutePath = "";
		        	 var imgSrc = imgs[i].getAttribute("src");
		        	 var Path = "/" + imgSrc.split("/")[1];
		        	 if(imgSrc.split("/")[0] != ""){
		        		 Path = "/" + imgSrc.split("/")[3];
		        		 absolutePath += imgSrc.split("/")[0];
		        		 absolutePath += "/" + imgSrc.split("/")[1];
		        		 absolutePath += "/" + imgSrc.split("/")[2];
		        		 imgs[i].setAttribute("_fcksavedurl",imgs[i].getAttribute("src").replace(absolutePath,""));
		        		 imgs[i].setAttribute("src",imgs[i].getAttribute("src").replace(absolutePath,""));
		        	 }
		        	 if(contextPath!=null&&contextPath!=""){
			        	 if(Path != contextPath){
			        		 imgs[i].setAttribute("_fcksavedurl",imgs[i].getAttribute("src").replace(Path,contextPath));
			        		 imgs[i].setAttribute("src",imgs[i].getAttribute("src").replace(Path,contextPath));
			        	 }
		        	 }
		         }
		     }
...
FCKEditingArea.prototype.Start = function( html, secondCall )
{
	...
	if ( this.Mode == FCK_EDITMODE_WYSIWYG )
	{
		...
		oIFrame.name = 'framesObj';
		oIFrame.id = 'framesObj';
