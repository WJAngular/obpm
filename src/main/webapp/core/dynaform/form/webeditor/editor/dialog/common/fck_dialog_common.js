...
{	
	if(!document.getElementById( elementId )){//firefox/chrome/safari
...
function CreateNamedElement( oEditor, oOriginal, nodeName, oAttributes )
{
	...
	var oldNode = null ;
	if ( oOriginal && oEditor.FCKBrowserInfo.IsIE && document.documentMode != 10)
	{
	...
	{
		// #676, IE doesn't play nice with the name or type attribute && !=ie10
		if ( oEditor.FCKBrowserInfo.IsIE && document.documentMode != 10)
		{
			var sbHTML = [] ;
	...
	for ( var n = 0 ; n < aAttributes.length ; n++ )
	{
		...
		if ( oAttribute.specified )
		{
			...
			try {
				...
				//alert("setAttribute failed ("+sAttName +": "+ sAttValue + ")");
			}
		}
	}
	// The style:
