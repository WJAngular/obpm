...
function selected(val, name){
	var rtn = '{"value":"' + val + '","name":"' + name + '"}';
	OBPM.dialog.doReturn(rtn);
}

</script>
</head>
<body id="application_module_print_list" class="body-back" style="overflow:hidden;">
<s:form name="formList" action="listforact" theme="simple" method="post">
		...
		<table width="120" align="right" border=0 cellpadding="0" cellspacing="0">
			<tr>
				<td width="60" valign="top">
					<button type="button" class="button-image" onClick="forms[0].action='<s:url action="newforact"></s:url>';forms[0].submit();"><img src="<s:url value="/resource/imgnew/act/act_2.gif"></s:url>">{*[New]*}</button>
				</td>
			</tr>
		</table>
		</td>
...
<div id="contentTable">
	<table class="table_noborder">
	   	<tr>
	   		...
	   		<td class="column-head"><o:OrderTag field="edit" css="ordertag">{*[Edit]*}</o:OrderTag></td>
		</tr>
		<s:iterator value="datas.datas" status="index">
				<tr>
	  	 	<td><a href="javascript:selected('<s:property value="id"/>','<s:property value="name"/>')" ><s:property value="name"/></a></td>
	  	 	<td><s:property value="description"/></td>
	  	 	<td><a href='<s:url value="editforact.action">
