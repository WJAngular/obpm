...
<script>
  function typeChange(type,index) { //'text','date','select','radio','textarea','checkbox'的改变
    ...
    var getField = function(data) {
	  	var s =''; 
		s +='<input type="text" class="fieldlength" id="fieldname'+ rowIndex +'" name="fieldname" value="'+HTMLDencode(data.fieldname)+'" onblur="checkSpace(this);checkStartChar(this);" size="20"/>';
		return s; 
	};
	...
	var getFieldLength=function(data){
	   var s =''; 
	   s +='<input type="text" class="fieldlength" id="fieldLength'+ rowIndex +'" onblur="checkNumber(this)" name="fieldLength" value="'+HTMLDencode(data.fieldlength)+'" size="10"/>';
	   return s; 
	};
	var getFieldOption=function (data){
	    var s=''; 
	 	s +='<input type="text" class="fieldlength" id="fieldOption'+ rowIndex +'" name="fieldOption" value="'+HTMLDencode(data.fieldoption)+'" size="20"/>';
	    return s; 
	};
	var getFieldFormat=function(data){
	 	var s='';
	 	s +='<input type="text" class="fieldlength" id="fieldFormat'+ rowIndex +'" name="fieldFormat" value="'+HTMLDencode(data.fieldformat)+'" size="10"/>';
	    return s;
	};
...
// 根据数据增加行
	...
	function createRelStr() {
		...
		for (var i=0;i<types.length;i++) {
			if (types[i].value != '' && fields[i].value!='') {
				...
				str += 'fieldformat:\''+ HTMLEncode(fieldformat[i].value) + '\',';
	...
	function ev_next() {
		...
		document.getElementsByName('content.f_fieldsdescription_sub')[0].value = createRelStr();
		document.forms[0].action='<s:url action="toStep2StyleSub"></s:url>';
	...
	function ev_init() {
		var str = document.getElementsByName('content.f_fieldsdescription_sub')[0].value;
...
<s:if test="runtimeException.nativeMessage !=null && runtimeException.nativeMessage !=''">	
	...
	<table width="99%" id="maintable">
		<tr style="height:100px;">
			...
			<td rowspan="2" style="width:200px;"><s:include value="wizard_common.jsp"><s:param name="step" value="26"/></s:include></td>
		</tr>
		<tr style="height:400px; vertical-align: top;">
			...
			<table id="contenttable" class="marAuto">
				...
				<tr>
					...
					<button type="button" onClick="forms[0].action='<s:url action="backToFormInfoSub" />';forms[0].submit();">{*[Back]*}</button>
					&nbsp;
					<button type="button" onClick="ev_next();">{*[Next]*}</button><s:textarea name="content.f_fieldsdescription_sub" id="fieldsDescription" cssStyle="display:none" />
					</td>
				</tr>
				<tr>
					<td colspan="2">
