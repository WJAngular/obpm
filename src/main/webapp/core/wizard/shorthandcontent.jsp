...
function dealwithProcess()
{
  ...
  def4 = document.getElementById('flowlist').value;
  ...
  { 
   if(document.all('_isbind')[i].checked&&document.all('_isbind')[i].value=='true')
    {
      flag ='1';
      document.getElementById('disFlow').style.display='';
      ActivityUtil.crateOnActionFlow(fn4,def1,mdlid,def4,function(str) {var func=eval(str);func.call()});
    }
   }   
    if(flag=='0')
    {
      document.getElementById('disFlow').style.display='none';
    }
}
...
function changePage(sign)
{
  ...
  {
    document.getElementById('formpage').style.display = 'none';
    document.getElementById('workflowpage').style.display ='';
				...
				<div id="formpage" valign="top"  >
					<table width='100%' valign="top">
							...
							<td colspan="2">
                 				<table width="98%" class="list-table" valign="top" >
	                				<tr class="list-toolbar">
						               	...
						               	<td width="3"></td>
		             					<td width="155" class="text-label">{*[Wizard]*}</td>
		            					<td>
						...
						<tr>
							...
							<td align="left"><s:textfield cssClass="input-cmd"  name="name" theme="simple"/></td>
						</tr>
						<tr>
							<td class="commFont" >{*[Type]*}:</td>
							<td align="left"><s:select name="type" list="_FORMTYPE" theme="simple" /></td>		
						</tr>
						<tr>
							<td class="commFont"  >{*[Operator]*}:</td>
							<td align="left"><s:checkboxlist  name="_formact" list="_FORMACLIST" theme="simple"/></td>
						</tr>
						<tr>
						<td  class="commFont" >{*[CreateField]*}:</td>
						<td align="left"><input type="radio" name="disField" value="ture" />Yes  <input type="radio" name="disField" value="false" checked="ture"/>No</td>
						</tr>
						<tr>
							<td  class="commFont" >{*[Example]*}:</td>
							<td align="left"> Test1:<s:textfield cssClass="input-cmd"  name="test" readonly="true" theme="simple"/>   Test2:<s:textfield cssClass="input-cmd"  name="test2" readonly="true" theme="simple"/></td>
						</tr>				
					</table>
					<table>
						<tr align="center">
						 <td ><button type="button" width="80px" onClick="if(document.getElementById('name').value==''){alert('Please enter the form name!');return false;};changePage(2)">{*[Next]*}</button></td>
						<td> <button type="button"  width="80px" onClick="if(document.v('name').value==''){alert('Please enter the form name!');return false;};forms[0].action='<s:url value='/core/wizard/saveShortHand.action'/>';forms[0].submit();">{*[Complete]*}</button></td>
						<td> <button type="button"  width="80px" onClick="forms[0].action='<s:url action="cancel"></s:url>';forms[0].submit();">{*[Cancel]*}</button>
						 </td>
						</tr>
					</table>
				</div> 
         		...
         		<div  valign="top" id="viewpage" style="display:none" > 
					<table width='100%'  valign="top" cellpadding="0" cellspacing="0" >
	                	<tr>
			                 	...
			                 	<table width="98%" class="list-table" valign="top"  >
					                <tr class="list-toolbar">
						               	<td width="10" class="image-label"><img
							                src="<s:url value="/resource/image/email2.jpg"/>" /></td>
						               	<td width="3"></td>
					             		<td width="155" class="text-label">{*[Wizard----view]*}</td>
						            	<td>
							            	<table width="100%" border=1 cellpadding="0" cellspacing="0"
								            	class="line-position" valign="top">
								           		<tr >
					   	...
					   	<tr><td class="commFont">{*[Open_Type]*}:</td><td align="left"><s:select label="{*[cn.myapps.core.dynaform.view.open_type]*}" name="opentype"  theme="simple" list="_OPENTYPE" /></td></tr>
						<tr><td class="commFont">{*[Operator]*}:</td><td align="left"><s:checkboxlist  name="_viewact" list="_VIEWACLIST" theme="simple"/></td></tr>
	           		</table>
           			<table>
						<tr align="center">
						 <td><button type="button" 
								    onClick="changePage(1)">{*[Back]*}</button></td>
						<td> <button type="button" onClick="if(viewcheck()==''){changePage(3); }else{alert(viewcheck());return false;}">{*[Next]*}</button> </td>
						 <td><button type="button"  width="80px" onClick="if(viewcheck()==''){ }else{alert(viewcheck());return false;};forms[0].action='<s:url value='/core/wizard/saveShortHand.action'/>';forms[0].submit();">{*[Complete]*}</button></td>
						<td> <button type="button"  width="80px" onClick="forms[0].action='<s:url action="cancel"></s:url>';forms[0].submit();">{*[Cancel]*}</button>
						 </td>
						</tr>
					</table>
        		...
        		<div id="workflowpage" valign="top" style="display:none" >
						             		...
						             		<table width="100%" border=1 cellpadding="0"  cellspacing="0"
							             		class="line-position">
							           			<tr>
												    ...
												    <td class="line-position2" width="70" valign="top">
												  <!--  <button type="button" class="back-class"
													  onClick="if(document.getElementById('workflowname').value==''){alert('Please enter the workflowname!');return false;};forms[0].action='<s:url action="saveShortHand"></s:url>';forms[0].submit();">{*[Complete]*}</button>
												     -->
												    </td>
										       	</tr>
									        </table>
					...
					<table>
						<tr align="center">
							<td>
						 		<button type="button" 
								    onClick="changePage(2)">{*[Back]*}</button></td>
						 	<td><button type="button" onClick="if(document.getElementById('workflowname').value==''){alert('Please enter the workflowname!');return false;};forms[0].action='<s:url value='/core/wizard/saveShortHand.action'/>';forms[0].submit();">{*[Complete]*}</button></td>
