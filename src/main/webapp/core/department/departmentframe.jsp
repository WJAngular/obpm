...
function departlist(){
	...
	//alert(departmentId);
	var page = contextPath + "/core/department/departmentlist.action?domain=<%=request.getParameter("domain")%>&sm_name="+sm_name+"&t_superior.id="+superiorid;
	jQuery("#userFrame").attr("src", page);	
}
function donew(){
	document.getElementById("selectnode").value="";
	document.getElementById("isclick").value ="true";
	var page = contextPath + "/core/department/new.action?domain=<%=request.getParameter("domain")%>&_currpage=<s:property value="datas.pageNo"/>&_pagelines=<s:property value="datas.linesPerPage"/>&_rowcount=<s:property value="datas.rowCount"/>";
	jQuery("#userFrame").attr("src", page);	
}
		...
		treeview = jQuery("#tree").jstree({ 
			...
			contextmenu :{
				...
				"add" : {
				    ...
				    "action"			: function(node){
					                        var page = contextPath + "/core/department/new.action?domain=<%=request.getParameter("domain")%>&_superiorid="+node.attr("id");
					                        jQuery("#userFrame").attr("src", page);
				                         }
					   },
			   "edit" : {
				    ...
				    "action"			: function(node){
					                        var page = contextPath + "/core/department/edit.action?domain=<%=request.getParameter("domain")%>&id="+node.attr("id");
					                        jQuery("#userFrame").attr("src", page);
				                         }
					   },
			    "delete" : {
					    ...
					    "action"			: function(node){	
						                            ...
						                            var url = contextPath+"/core/department/treeDelete.action?domain=<%=request.getParameter("domain")%>&departmentids="+ids+"&datetime=" + new Date().getTime(); 					                        
						                            ...
						                            var page = contextPath + "/core/department/departmentlist.action?domain=<%=request.getParameter("domain")%>";
							                        jQuery("#userFrame").attr("src", page);
						                            }
					                         }
						}
			...
			if (node && node.attr) {
				if (selectedNode != node.attr("id") && !isCheckEl) { // 是否刷新右窗口	
			      ...
			      var page = contextPath + "/core/department/departmentlist.action?domain=<%=request.getParameter("domain")%>&t_superior.id="+node.attr("id");
			      jQuery("#userFrame").attr("src", page);
		...
		DepartmentHelper.getDepartmentList(domain, function(data){
			if(data!=null){
				...
				for(var i=0; i<departments.length;i++){
					var url = contextPath + '/core/department/departmentlist.action?domain='+'<%=request.getParameter("domain")%>';
...
<div class="ui-layout-west" style="overflow: auto;">
 <!-- 原来的jsTree-->
  ...
  <!-- 以下为dtree 
	...
	<div id="departmentTree" style="overflow:auto;position: relative;"></div>
	-->
</div>
...
<div class="ui-layout-center">
	<div id="main">
		<div id="contentTable" style="height:100%;overflow-y:auto;">
			<iframe width="100%" height="100%" scrolling="auto" src="<%=request.getContextPath() %>/core/department/departmentlist.action?domain=<%=request.getParameter("domain")%>"
