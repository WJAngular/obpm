...
(function($) {
	...
	$.fn.colorPicker=function(vIn) {
		...
		if (cp.length==0) {
			...
			cp.css({"position":"absolute","z-index":"1000","background-color":"#FFFFFF","border":"1px solid #CCCCCC","padding":"1px","cursor":"pointer","display":"none"});
			...
			if($.browser.msie&&cf.length==0) {
				cf=$('<iframe scrolling="no" frameborder="0" style="position:absolute;z-index:9900;display:none" id="fy_CP_Frame"></iframe>');
				cf.appendTo('body');
			}
		}
