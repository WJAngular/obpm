 ...
 */
		...
		$.each(c.borderPanes.split(","), function() {
			...
			if (o.closable) {
				...
				$T
					...
					.click(function(evt){if(jQuery(this).attr("class").indexOf("arrow1") == "-1"){jQuery(this).removeClass("arrow");jQuery(this).addClass("arrow1");}else{jQuery(this).removeClass("arrow1");jQuery(this).addClass("arrow");} toggle(pane); evt.stopPropagation();})
					...
					.addClass("arrow " +  tClass +" "+ tClass+_pane +" "+ tClass+_state +" "+ tClass+_pane+_state )
	...
	var initResizable = function () {
		...
		$.each(c.borderPanes.split(","), function() {
			...
			$R.draggable("disable");//初始化时设置不可拖动
		});
	};
 ...
 */
	...
	var bindStopSlidingEvents = function (pane, enable) {
		...
		if (enable) { // BIND trigger events
			...
			$P.unbind(trigger);
			$R.unbind(trigger);
			$R.find("div").removeClass("arrow1");
			$R.find("div").addClass("arrow");
			if (trigger = "mouseout") {
	...
	var slideOpen = function () {
		jQuery(this).find("div").removeClass("arrow");
		jQuery(this).find("div").addClass("arrow1");
			...
			if (pane == "center") {
				...
				if(CSS.width>100000){
					if(parent.document.body){
						CSS.width=$(parent.document.body).width()-CSS.left-2;
					}
				}
