...
function ev_select(key,value,isChecked,obj) {
	...
	if (isChecked) {
		...
		if(flag){
			jQuery(".viewRightInfo").append("<tr id='right_" + key + "' class=\"listDataTr\">"+htmls+"</tr>");
	...
	<table width="100%" cellspacing="10">
	    ...
	    <tr>
	    	<s:if test="(#parameters.mutil[0] == 'true'||#parameters.selectOne[0] == 'true')&&#parameters.isEdit[0] == 'true'">				
				...
				onClick="emptycheck()">{*[cn.myapps.core.dynaform.view.ClearFile]*}</a></td>
			</s:if>
			<s:if test="#parameters.isEdit[0] == 'true'">
				<td><a class="btn btn-positive btn-block" data-transition="fade" 
				onClick="deletechecked()">{*[cn.myapps.core.dynaform.view.delete_checked]*}</a></td>
			</s:if>	
			<td><a class="btn btn-positive btn-block" data-transition="fade" onClick="viewSelectBtn()">返回</a></td>
		</tr>
		</tbody>
	</table>
</div>
...
<div id="selectBtn" class="card_space_fix zindex10">
	<table width="100%" cellspacing="10">
	    ...
	    <tr>
			...
			<td><a id="dialog_selectBtn" class="btn btn-positive btn-block" data-transition="fade" onClick="viewSelectBtn()">选择</a></td>
		</tr>
		</tbody>
	</table>
</div>	
