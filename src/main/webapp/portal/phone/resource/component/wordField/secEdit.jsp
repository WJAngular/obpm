...
<%
  ...
  if(request.getAttribute("secFiles")!=null){
	  files = (File[])request.getAttribute("secFiles");
  }
%>
  ...
  <body onload="init()">
    ...
    <form id="secForm" method="post" enctype="multipart/form-data" action='<s:url action="uploadfile.action" namespace="/portal/upload"/>'>
     ...
     <table id="tb" border="1" cellspacing="0" align="center">
       ...
       %>
         <tr>
           <td align="center"><%=files[i].getName()%></td>
           <td align="center"><%=new Date(files[i].lastModified()).toLocaleString()%></td>
           <td align="center"><%=files[i].length()%></td>
           <td align="center"><a href="javascript:editesp('<s:property value='#request.secPath'/><%=files[i].getName()%>')">编 辑</a></td>
         </tr>
       <%
         }
       %>
