 ...
 * Copyright 2011 by Marek Biela艅czuk
	...
	$.fn.gantt = function(options) {
		...
		var core = {
			...
			render: function (element) {
				...
				content.append(core.createSwitch(element));
				...
				var ganttTitleH = "";
				$(".ganttTitle").each(function(){
					var $ganttTitle = $(this);
					if($ganttTitle.html() != ""){
						if(ganttTitleH == ""){
							ganttTitleH = $ganttTitle.height();
						}else{
							ganttTitleH = ganttTitleH + $ganttTitle.height();
						}
					}
				});
				$(".spacer").height(ganttTitleH);
				...
				core.markNow(element);	//标识今天
				core.fillData(element);	//添加状态条和依赖关系
			 ...
			 * 创建视图切换操作区
			 */
			createSwitch : function(element) {
				var $actionBtn = $('<div style="padding: 5px 0 2px 10px;"></div>')
					.append($('<input type="radio" name="switchView" value="months"'
							+(settings.scale == 'months' ? ' checked="checked"' : '')+'/>').bind('click',function(){
						settings.scale = this.value;
						core.init(element);
					}))
					.append(ganttText.month)
					.append($('<input type="radio" name="switchView" value="weeks"'
							+(settings.scale == 'weeks' ? ' checked="checked"' : '')+'/>').bind('click',function(){
						settings.scale = this.value;
						core.init(element);
					}))
					.append(ganttText.week)
					.append($('<input type="radio" name="switchView" value="days"'
							+(settings.scale == 'days' ? ' checked="checked"' : '')+'/>').bind('click',function(){
						settings.scale = this.value;
						core.init(element);
					}))
					.append(ganttText.day)
					.append($('<input type="radio" name="switchView" value="hours"'
							+(settings.scale == 'hours' ? ' checked="checked"' : '')+'/>').bind('click',function(){
						settings.scale = this.value;
						core.init(element);
					}))
					.append(ganttText.hours)
					.append($('<div class="actionBtn">'+ganttText.openAll+'</div>').bind('click',function(){
						core.showHideAllToggle(element,'show');
					}))
					.append($('<div class="actionBtn">'+ganttText.flodAll+'</div>').bind('click',function(){
						core.showHideAllToggle(element,'hide');
					}));
				return $actionBtn;
			},
			leftPanel: function (element) {
				/* Left panel */
				var ganttLeftPanel = $('<div class="leftPanel" style="width:'+settings.leftWidth+';"/>')
				...
				$.each(element.data, function(i, entry) {
					...
					{
						var $label = $('<div class="label" style="word-break:break-all;height:23px;overflow:hidden;"/>');
						//添加空格位
						var spaceDiv = '';
						for(var z=0; z<entry.values[0].rankSize; z++){
							spaceDiv += '<div class="space"></div>';
						}
						$label.append(spaceDiv);
						
						//添加切换显示/折叠开关
						if(entry.values[0].isPar){
							var $objSwitch = $('<div class="switch" cId="'+entry.values[0].id+'">&nbsp;</div>')
								.bind('click',function(){
									var state = '';
									if($(this).hasClass('closeSub')){
										state = 'show';
									...
									core.showHideToggle(element,this,state);
								});
							$label.append($objSwitch);
						}
						if(typeof(entry.name) != "undefined"){
							$label.append('<input type="checkbox" name="_selects" value="'+entry.values[0].id+'"/>'
								+ '<a href="javascript:viewDoc(\''+ entry.values[0].id +'\',\''
								+ entry.values[0].formId + '\')" title="' + entry.name + '">' + entry.name + '</a>');
							
							ganttLeftPanel
								.append($('<div class="row name row' + i + ' " pId="'+entry.values[0].pId+'"/>')
										.append($label))
								.append($('<div class="row desc row' + i + ' " pId="'+entry.values[0].pId+'"/>')
										.append($('<span class="label" title="' + entry.desc + '"/>').html(entry.desc)));
						}
					}
				});
			...
			rightPanel: function(element) {
				...
				var years = $("<div class='row ganttTitle'/>");
				var daysInYear = 0;

				var months = $("<div class='row ganttTitle'/>");
				var daysInMonth = 0;
				...
				switch (settings.scale)
				{
					...
					case 'hours':
						...
						dataPanel.append($('<div class="row ganttTitle"/>').html(daysStr));
						dataPanel.append($('<div class="row ganttTitle"/>').html(dowStr));
						dataPanel.append($('<div class="row ganttTitle"/>').html(horStr));
						...
						for (var i=0; i<element.data.length; i++)
						{
							...
							if (i >= element.pageNum*settings.itemsPerPage && i < (element.pageNum*settings.itemsPerPage+settings.itemsPerPage))
							{
								var dRow = '<div class="row" pId="'+entry.values[0].pId+'">';
				...
				return $('<div class="rightPanel" style="width:'+settings.rightWidth+';"></div>').append(dataPanel); 
			},
					...
					ganttNavigate = $('<div class="navigate" />')
						.append($('<div class="nav-slider" />')
							.append($('<div class="nav-slider-left"/>')
								.append($('<a href="javascript:///" class="nav-link nav-page-back" title="'+ganttText.prevPage+'"/>')
									.html('&lt;')
									.click( function () {
								...
								.append($('<a href="javascript:///" class="nav-link nav-page-next" title="'+ganttText.nextPage+'"/>')
									.html('&gt;')
									.click( function () {
										core.navigatePage(element, 1);
									}))
								.append($('<a href="javascript:///" class="nav-link nav-now" title="'+ganttText.today+'"/>')
									.html('&#9679;')
									.click( function () {
										core.navigateTo(element, 'now');
									}))
								.append($('<a href="javascript:///" class="nav-link nav-prev-day" title="'+ganttText.leftSlide+'"/>')
									.html('&lt;')
									.click( function () {
							...
							.append($('<div class="nav-slider-right" />')
								.append($('<a href="javascript:///" class="nav-link nav-next-day" title="'+ganttText.rightSlide+'"/>')
									.html('&gt;')
									.click( function () {
...
//									}))
					...
					ganttNavigate = $('<div class="navigate" />')
						...
						.append($('<a href="javascript:///" class="nav-link nav-prev-week" title="'+ganttText.leftSlide+'"/>')
							.html('&lt;&lt;')
							.click( function () {
						...
						.append($('<a href="javascript:///" class="nav-link nav-next-week" title="'+ganttText.rightSlide+'"/>')
							.html('&gt;&gt;')
							.click( function () {
								core.navigateTo(element, tools.getCellSize() * -7);
							}))
...
//							}));
			...
			createProgressBar: function(days, bID, cls, desc, label, bgcolor,complete) {
				var cellWidth = tools.getCellSize();
				var barMarg = tools.getProgressBarMargin() || 0;
				var barId = bID ? "id='" + bID + "'" : "";
				var bar = $("<div "+ barId +" class='bar'>" +
						"<div class='label'>"+label+"&nbsp;&nbsp;<span style='font-weight:normal;'>"+complete+"%</span></div>" +
						"</div>")
						.addClass( cls )
						.css({
...
;
			 ...
			 * @param cObj
			 */
			showHideToggle: function(element,cObj,state){
				$(element).find('div[pId='+$(cObj).attr('cId')+']').each(function(){
					if(state == 'show'){
						$(this).show(300);
					}else if(state == 'hide'){
						$(this).hide(300);
					}
					if (!$(this).find('.switch').hasClass('closeSub')) {//子任务隐藏时不递归
						$(this).find('div.switch').each(function(){
							core.showHideToggle(element,this,state);
						});
					}
				});
			 ...
			 * 展开和折叠所有分组
			 */
			showHideAllToggle: function(element,state){
				$(element).find('div.switch').each(function(){
					if(state == 'show'){
						$(this).removeClass('closeSub');
					}else if(state == 'hide'){
						$(this).addClass('closeSub');
					}
					$(element).find('div[pId='+$(this).attr('cId')+']').each(function(){
						if(state == 'show'){
							$(this).show(300);
						}else if(state == 'hide'){
							$(this).hide(300);
		...
		var tools = {
			getMaxDate: function(element) {
				...
				$.each(element.data, function(i, entry) {
					$.each(entry.values, function(i, date) {
						maxDate = maxDate < tools.dateDeserialize(date.to) || maxDate == null ? tools.dateDeserialize(date.to) : maxDate;
					});
				});
			...
			dateDeserialize: function (dateStr) {
				...
				return new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getDate(), date.getHours(), date.getUTCMinutes());
			},
