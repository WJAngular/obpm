 ...
 *
 * $Date: 2010-07-06 15:24:57 +0800 (星期二, 06 七月 2010) $
 * $Revision: 194 $
 */
	...
	$(function() {
		...
		var u = navigator.userAgent.toLowerCase(),
			...
			css_string = '' + 
				...
				'.jstree ins { vertical-align: bottom;display:inline-block; text-decoration:none; width:36px; height:36px; margin:0 10px 0 0; padding:0; } ' + 
				'.jstree a { border: 1px #8C8C8C solid;border-radius: 4px;display:inline-block; height:32px; color:black; white-space:nowrap; text-decoration:none; padding:5px; margin:0; } ' + 
				'.jstree a:focus { outline: none; } ' + 
				'.jstree a > ins { height:16px; width:16px;display:none } ' + 
		...
		if(/msie/.test(u) && parseInt(v, 10) == 6) { 
			is_ie6 = true;
			css_string += '' + 
				'.jstree li { height:36px; margin-left:0; margin-right:0; } ' + 
				'.jstree li li { margin-left:36px; } ' + 
				'.jstree-rtl li li { margin-left:0px; margin-right:36px; } ' + 
				'li.jstree-open ul { display:block; } ' + 
	...
	$.jstree.plugin("search", {
		...
		_fn : {
			search : function (str, skip_async) {
				...
				// 1.首次调用ajax
				if(!skip_async && s.ajax !== false && this.get_container().find("li.jstree-closed:eq(0)").length > 0) {
					this.search.supress_callback = true;
					error_func = function () { };
					success_func = function (d, t, x) {
						var sf = this.get_settings().search.ajax.success; 
						if(sf) { d = sf.call(this,d,t,x) || d; } // 1.1获取返回的对象
...
(function ($) {
	...
	$(function () {
		var css_string = '' + 
			'#vakata-contextmenu { display:none; position:absolute; margin:0; padding:0; min-width:110px; background:#ebebeb; border:1px solid silver; z-index:10000; *width:110px; } ' + 
			'#vakata-contextmenu ul { min-width:100px; *width:100px; } ' + 
					...
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						...
						success_func = function (d, t, x) {
							...
							if(d) {
								...
								else { obj.children("a.jstree-loading").removeClass("jstree-loading"); obj.append(d).find("li, a").filter(function () { return this.firstChild.tagName !== "INS"; }).prepend("<ins class='jstree-icon'>&#160;</ins>").end().filter("a").children("ins:first-child").not(".jstree-icon").addClass("jstree-icon"); obj.data("jstree-is-loading",false); }
								this.clean_node(obj);
								if(s_call) { s_call.call(this); }
							}
							else {
...
(function ($) {
	...
	$(function() {
		var css_string = '' + 
			'.jstree .ui-icon { overflow:visible; }' + 
			'.jstree .jstree-no-icon { display:none; }';
		$.vakata.css.add_sheet({ str : css_string });
	});
})(jQuery);
//*/
