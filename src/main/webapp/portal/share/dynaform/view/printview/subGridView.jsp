...
function tb_Size(){
	var gridTableW = jQuery("#gridTable").width();
	jQuery("body").width(gridTableW -2);
	jQuery("#dspview_divid").width(gridTableW-2);
	jQuery("#oLayer").width(gridTableW-2);
}
...
%>
	...
	<table id="gridTable" class="display_view-table" style="border-top:1px solid #b5b8c8;position:relative; z-index:1;" border="0" cellpadding="0" cellspacing="0" width="100%">	
		<!-- table-header -->
		...
		<%
			...
			out.print(view.printToRowHtml(doc, jsrun, user,isEdit,null));
		%> 
		</s:iterator>
