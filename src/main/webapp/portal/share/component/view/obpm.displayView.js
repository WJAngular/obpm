...
function dataTableSize(){
	//包含元素固定高度时不重新布局
	var divid = $("[name='divid']").eq(0).innerText;
	var pSpan = $("#"+divid);
	var pIframe = "";
	var fixation = "false";
	if(pSpan){
		pIframe = pSpan.find("iframe").eq(0);
		if(pIframe){
			fixation = pIframe.attr("fixation");
		}
	}
				...
				if(i == 0){//首行（列头）
					...
					$trField.children().each(function(i){//单元格<td>
						...
						if(i == 0){//首列
							...
							$trHtml.append(toDataTdHtml($tdField));//重构数据单元格
						}
					});
				}
...
var DisplayView = {
	...
	addListen : function(){
		...
		$(".btn-jumppage").on("click",function(e){
			...
			var pageCount = Math.ceil(parseInt(json["_rowcount"])
					/ parseInt(json["_pagelines"]));
			var _currpage = parseInt($("input[name='_jumppage']").val());
			if(_currpage>pageCount) return;
			json["_currpage"] = _currpage;
			var url = DisplayView.getAction(this);
			DisplayView.postForm(this,url,json);
