...
<script type="text/javascript">
	...
	function initDoReturnFunction(){
		jQuery("#doReturn").click(function(){
			...
			jQuery(".onSelect").each(function(){
				rtn+='U'+jQuery(this).attr("id")+"|"+jQuery(this).attr("name")+";";
			});
			if (rtn && rtn.length > 0) {
	...
	function addToUserSelectByMap(map){
		var id = map["id"];
		var name = map["name"];
		jQuery("#selectedUserDiv").append("<span style='display:block;width:95%;' onclick='clickRemoveSelect(\""+id+"\",\""+name+"\")' title='{*[Click]*}{*[Delete]*}' class='onSelect' id="+id+"  name="+name+">"+name+"</span>");
	}
...
<body style="overflow:hidden;margin: 0;padding: 5px;height:95%;height:100%\9;">
