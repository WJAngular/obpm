...
function importDocument(){
    ...
    formItem.action = "<s:url value='/portal/domain/excelImportUserAndDept.action'/>";
    formItem.submit();
}

function uploadFrontSimple(pathname, pathFieldId, applicationid) {
	var url = contextPath+ '/portal/share/component/upload/upload.jsp?path=' + pathname + '&applicationid=' + applicationid;
	...
	<body>
	<s:form name="formItem" action="" method="post">
	<s:hidden name="domainid"></s:hidden>
		<table border="0">
			<tr>
				<td class="commFont">{*[Attachment]*}:&nbsp;&nbsp;</td>
				<td class="commFont" colspan="2">
    				<a href="javascript:download();"><font color="#0000FF">{*[cn.myapps.core.dynaform.dts.excelimport.click_download_template]*}</font></a>
				</td> 
			</tr>
			<tr>
