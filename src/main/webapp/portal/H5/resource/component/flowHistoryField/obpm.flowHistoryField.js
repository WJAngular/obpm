		...
		return this.each(function(){
			...
			$html.find(".signatureImage").on("click",function(){
				...
				if($("body").find("#signatureImage-preview").size() <= 0){
					$("body").append("<div id='signatureImage-preview' style='position: fixed;top: 0px;bottom: 0px;left: 0px;right: 0px;background: rgba(0, 0, 0, 0.5);z-index: 1000;'>" +
							"<div class='signatureImage-panel' style='display: table;width: 100%;height: 100%;text-align: center;'>" +
							"<div class='signatureImage-pic' style='display: table-cell;vertical-align: middle;'><img title='点击关闭' style='background: #fff;'/></div>" +
							"</div>" +
							"</div>");
				}		
