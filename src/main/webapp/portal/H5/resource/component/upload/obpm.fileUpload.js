(function($) {
	...
	function getDivContent(fileFullName, uploadListId, readonly, refresh,
		...
		for ( var i = 0; i < files.length; i++) {
			...
			tohtml += '<span class="showOptions btn btn-default" style="display:inline-block;position:relative;z-index:1;margin-right:5px;">';
	...
	$.fn.obpmUploadField = function() {
		return this.each(function() {
			...
			if(!readonly){//只读状态下不显示上传和下载按钮
				// uploadinput
				if (limitType == "image") {
					var uploadHtml = "<div class='uploadinput' " +
							"style='height:"+imgHeight+"px; width:"+imgWidth+"px; display: table-cell; vertical-align: middle; text-align: center;cursor: pointer;border: 3px dashed #e6e6e6;color: #e6e6e6;font-size: 24px;'>" +
							"<span class='glyphicon glyphicon-picture' aria-hidden='true'></span>" +
							"</div>";
					
				}else{
					var uploadHtml = "<input class='uploadinput btn btn-info' type='button'  value='"
						+ uploadLabel + "' id='"+id+"_upload' limitNumber='"+limitNumber+"' name='btnSelectDept'";
				...
				if (limitType == "image") {
					...
					var delHtml = "<input class='uploaddelete btn btn-danger' type='button' name='btnDelete'  value='"
						+ deleteLabel + "'";
					if (disabled && disabled != '') {
			...
			if (limitType == "image") {
				...
				var td2Html = "<div style='display: inline-block;'><div  id='"
						+ uploadList
