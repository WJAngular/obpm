...
<script type="text/javascript">
			...
			jQuery.ajax({
					...
					success:function(data){
							if(data){
								...
								jQuery(".crossULdivleft").css({"width":"423px","overflow":"auto"});
								jQuery(".guideOne").css("display","none");
								jQuery(".crossULdivright_lef").css("display","none");
								jQuery("#leftcontent").html(data);*/
							}else{
								jQuery("#SHvalue").attr("value",param);
	...
	function getUserListByRoleid(roleObj){
		...
		if(rolesid!=""){
			jQuery("#righttitle").html("{*[Role]*}："+rolesname).attr("title", rolesname);
			jQuery.ajax({
	...
	function getUserListByDept(domainid,deptid,deptname){
		if(domainid!="" && deptid!=""){
			jQuery("#righttitle").html("{*[Department]*}:"+deptname).attr("title", deptname);
			jQuery.ajax({
	...
	function getUserListByContanctsid(contanctsObj){
		...
		if(contanctsid!=""){
			jQuery("#righttitle").html("分组："+contanctsname).attr("title", contanctsname);
			jQuery.ajax({
	...
	jQuery(document).ready(function(){
		...
		jQuery("#addAll").click(function(){
			...
			/**
			var errorMsgBeAdd = '';
			jQuery("#rightcontent").find('input.list_div_click').each(function(){
...
<body style="overflow-x:hidden;overflow-y:hidden;height:100%;margin: 0;">
	...
	<div class="crossUL-top">
			<ul class="crossUL">
				...
				<li id="byroles" >{*[cn.myapps.core.email.write.choose.tab.title.by_role]*}</li> 
