...
(function($){
	...
	function _init(t){
		...
		//var v = t.val()
		var panel = $('<div id="survyfield_container_'+options.id+'"></div>');
	...
	function _createAnswerPanle(question,displayType){
		...
		for(i in question.options){
			...
			case "radio":
				...
				break;
			case "text":
				$o = $('<tr id="question-'+question.id+'-answerrow-'+i+'"><td style="height: 26px; padding-right: 10px;"><input id="question-'+question.id+'-answer-'+i+'" name="question-'+question.id+'-answer" ordernumber="'+i+'" type="'+type+'" data-type="'+type+'" data-custom="false" data-text="'+text+'" value="'+value+'" onclick="" '+disabled+'></td> </tr>');
				break;
			case "textarea":
				$o = $('<tr id="question-'+question.id+'-answerrow-'+i+'"><td style="height: 26px; padding-right: 10px;"><textarea id="question-'+question.id+'-answer-'+i+'" name="question-'+question.id+'-answer" ordernumber="'+i+'" data-type="'+type+'" data-custom="false" data-text="'+text+'" onclick="" '+disabled+'>'+value+'</textarea></td> </tr>');
				break;
			default:
				break;
			}
	...
	function _getValue(jq){
		...
		for(i in  questions){
			...
			panel.find("input[name='question-" + question.id + "-answer']:checked" +
					", input[name='question-" + question.id + "-answer'][type='text']" +
					", textarea[name='question-" + question.id + "-answer']").each(function(index){
				var $t = $(this);
	...
	function _setValue(jq,v){
		...
		for(i in  values){
			...
			panel.find("input[name='question-"+questionId+"-answer'],textarea[name='question-"+questionId+"-answer']").each(function(index){
				var $t = $(this);
				for(k in answers){
					...
					} else if(answer.type == "text" && answer.text==$t.data("text")){
						$t.val(answer.value);
						break;
					} else if(answer.type == "textarea" && answer.text==$t.data("text")){
						$t.val(answer.value);
						break;
					}
				}
