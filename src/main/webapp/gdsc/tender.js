$(function(){
	...
	if($("input[name='资金用途']:checked").val() == "其它"){
		showItem("其他_");
	}else{
		hideItem("其他_");
	}
	//当点击时，触发显示或隐藏
	$("input[name='资金用途']").click(function(){
		var ware = $("input[name='资金用途']:checked").val();
		if(ware == "其它"){
			attrItem("其他_","placeholder","其它");
			showItem("其他_");
		}else{
			hideItem("其他_");
		}
	...
	$("a[title='打印']").unbind('click').bind('click',function(){
		...
		if(docNo == ""){
			...
			window.open("http://192.168.1.30:8080/obpm/ReportServer?reportlet=ztbPrint.cpt&number="+docNo);
		}
	});
});
...
var $this = {
	'defaultHideValue':function(){
			attrItem("费用类型","value","招投标费");
			attrItem("招投标费申请","value","招投标费申请");
		},
