			...
			ok : function() {
	        	if(validateEmpty()){
		        	$.ajax({
		        		...
		        		success:function(data){
		        			artDialog.close();
		        			window.location.href='<s:url value="/km/baike/center/index.jsp?userId=" />';
		        		},
		        		error:function(data) {
		        			artDialog.close();
		        			alert('修改失败!');
			...
			<div>
            	<div id="content_left" style="float:left;margin-left:20px; width:810px;height=100%;overflow:hidden;" class="bod">
					<div class="bk_title_body">
						...
						<div class="vote_btn vote_btn_1" title="通过">
							<a href='<s:url value="/km/baike/center/doApprove1.action" />?contentId=<s:property value="content.content.id"/>'>通过</a>
						</div>
						
						<div id="bdshare"  title="点击驳回" class="bdshare_t bds_tools get-codes-bdshare bk_share log-set-param" >
							<span class="bds_more">
								<a onclick="setDialogContent('reasonDialog','<s:property value="content.content.id" />');mydialog();">
								驳回</a>
							</span>
						</div>
					</div>
					...
					<div style=" font: 14px/1.8 arial,宋体,sans-serif;">	
						<s:property value="content.content.content" escape="false"/>
