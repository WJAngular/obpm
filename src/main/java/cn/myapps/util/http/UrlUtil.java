...
public class UrlUtil {
	 ...
	 * @return
	 */
	public static String parameterize(String uri, String paramsName,
			String paramsValue) {
			...
			if (entry.getValue() != null)
				if (entry.getValue().getClass().isArray()) {
					...
					for (int j = 0; j < value.length; j++) {
						...
						buffer.append('=');
						try {
							buffer.append(URLEncoder.encode((String) value[j],
									"UTF-8"));
					...
					buffer.append('=');
					try {
						buffer.append(URLEncoder.encode((String) entry
								.getValue(), "UTF-8"));
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
	...
	public static String deparameterize(String uri,
			Map<Object, Object> parameters, boolean isReplace) {
	...
	public static void main(String[] args) throws Exception {
		...
		// 将Map中的参数添加到URL
		String newURL = UrlUtil.parameterize("/obpm/portal?bbb=bbbvalue",
				parameters);
		...
		System.out
				.println(getServerName("http://localhost:8080/obpm-cas/login"));
		System.out
				.println(getUrlPrefix("http://localhost:8080/obpm-cas/login"));
	}
}
