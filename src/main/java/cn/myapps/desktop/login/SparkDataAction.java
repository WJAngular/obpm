 ...
 * 
 */
public class SparkDataAction extends ActionSupport {
	...
	public String getMenuList() throws Exception {
		...
		if (pwdErrorTimes != null) {
			loginPasswordErrortimes = Integer
					.parseInt(pwdErrorTimes.getValue());
		}
		...
		try {
			...
			if (user != null && user.getStatus() == 1) {
				...
				if (user.getUserSetup() != null) {
					session.setAttribute(Web.SKIN_TYPE, user.getUserSetup()
							.getUserSkin());
				} else {
	...
	public String getPendingList() throws Exception {
		Cookie pwdErrorTimes = getErrorTimes();
		int loginPasswordErrortimes = 0;
		if (pwdErrorTimes != null) {
			loginPasswordErrortimes = Integer
		...
		try {
			...
			if (user != null && user.getStatus() == 1) {
				...
				Environment.getInstance().setContextPath(
						ServletActionContext.getRequest().getContextPath());
				String toXml = getPendingXmlByApplication2(webUser)
						+ getUserInfoXml();
				if (toXml != null) {
					toXml = "<" + MobileConstant.TAG_MAIN + " "
							+ MobileConstant.ATT_SESSIONID + "='"
	...
	public String getUserList() throws Exception {
		...
		try {
			...
			if (user != null && user.getStatus() == 1) {
				...
				Environment.getInstance().setContextPath(
						ServletActionContext.getRequest().getContextPath());
				String toXml = getUserListXml(user.getDomainid())
						+ getUserInfoXml();
				if (toXml != null) {
					toXml = "<" + MobileConstant.TAG_MAIN + " "
							+ MobileConstant.ATT_SESSIONID + "='"
		...
		try {
			...
			if (toXml != null) {
				toXml = toXml.replaceAll(
						"<" + MobileConstant.TAG_APPLICATION + ">",
						"<" + MobileConstant.TAG_REFRESHAPPLICATION + ">")
						.replaceAll(
								"</" + MobileConstant.TAG_APPLICATION + ">",
								"</" + MobileConstant.TAG_REFRESHAPPLICATION
										+ ">");
			...
			// System.out.println("login->:" + toXml);
			long end = System.currentTimeMillis();
			System.out.println("doRefreshApplication Waste Time: "
					+ (end - time) + "(ms)");
		} catch (Exception e) {
			LOG.warn(e);
			this.addFieldError("SystemError", e.getMessage());
			...
			for (Iterator<ApplicationVO> it = apps.iterator(); it.hasNext();) {
				...
				if (app != null && !StringUtil.isBlank(app.getId())) {
					...
					for (Iterator<ResourceVO> it1 = topMenus.iterator(); it1
						...
						// temp = getSubMenus(resvo.getId(), app.getId(),
						// getUser().getDomainid());
	...
	private String menuRecursive(ResourceVO resvo, Collection<ResourceVO> coll,
		...
		for (Iterator<ResourceVO> it1 = coll.iterator(); it1.hasNext();) {
			...
			// Collection<ResourceVO> subMenus =
			// process.doGetDatasByParent(resvo1
			// .getId());
		...
		try {
			...
			for (Iterator<ApplicationVO> it = apps.iterator(); it.hasNext();) {
				...
				try {
					...
					params1.setParameter("i_useddefined",
							UserDefined.IS_DEFINED);
					params1.setParameter("_orderby", "id");
					DataPackage<UserDefined> dataPackage = udprocss
							.doQuery(params1);
					if (dataPackage.rowCount > 0) {
						UserDefined userDefined = new UserDefined();
						for (Iterator<UserDefined> ite1 = dataPackage.datas
								.iterator(); ite1.hasNext();) {
							userDefined = (UserDefined) ite1.next();
						}
						xml.append(createTemplateElement(app.getId(),
								userDefined, webUser));
						...
						if (dataPackage1.rowCount > 0) {
							for (Iterator<UserDefined> ite1 = dataPackage1.datas
									.iterator(); ite1.hasNext();) {
								UserDefined userDefined = (UserDefined) ite1
										.next();
								// 判断是否适用于所有角色
								if ("1".equals(userDefined.getDisplayTo())) {
									xml.append(createTemplateElement(app
											.getId(), userDefined, webUser));
								} else {
									// 获取某一首页的角色
									String roleIds = userDefined.getRoleIds();
									if (!StringUtil.isBlank(roleIds)) {
										String[] userRoleIds = roleIds
												.split(",");
	...
	public static SummaryCfgVO summaryIdCheck(String summaryid)
		...
		try {
			...
			if (summaryCfg == null) {// 兼容旧数据
				...
				if (widget != null
						&& PageWidget.TYPE_SUMMARY.equals(widget.getType())) {
					summaryCfg = (SummaryCfgVO) summaryCfgPro.doView(widget
							.getActionContent());
				}
			}
		} catch (Exception e) {
	...
	private String createTemplateElement(String id, UserDefined userDefined,
					...
					for (int j = 0; j < summaryIds.length; j++) {
						...
						if (summaryCfg.getScope() == SummaryCfgVO.SCOPE_PENDING) {// 代办
							...
							DataPackage<PendingVO> pendings = pendingProcess
									.doQueryByFilter(summaryCfgParams, user);
							for (Iterator<PendingVO> iterator = pendings.datas
									.iterator(); iterator.hasNext();) {
								PendingVO pendingVO = (PendingVO) iterator
										.next();
								xml
										.append("<"
												+ MobileConstant.TAG_PENDING_ITEM
												+ " ");
								...
								String isread = "false";
								Collection<ActorRT> actors = pendingVO
										.getState().getActors();
								for (Iterator<ActorRT> iter = actors.iterator(); iter
										.hasNext();) {
							...
							CirculatorProcess circulatorProcess = (CirculatorProcess) ProcessFactory
									.createRuntimeProcess(
											CirculatorProcess.class, id);
							DataPackage<Circulator> circulators = circulatorProcess
									.getPendingByUser(circulatorParams, user);
							for (Iterator<Circulator> iterator = circulators.datas
									.iterator(); iterator.hasNext();) {
								Circulator circulator = (Circulator) iterator
										.next();
								xml
										.append("<"
												+ MobileConstant.TAG_PENDING_ITEM
	...
	private void sort(Collection<ResourceVO> temp) {
		Collections.sort((ArrayList<ResourceVO>) temp,
				new Comparator<ResourceVO>() {
