package cn.myapps.desktop.user;
...
import cn.myapps.desktop.personal.DPersonalAction;
...
public class DUserAction extends DPersonalAction{
	...
	private String getUserXML(BaseUser uservo) throws Exception{
		...
		sb.append("<" + MobileConstant.TAG_TEXTFIELD + " " + MobileConstant.ATT_NAME + "='uservo.defaultDepartmentName' " + MobileConstant.ATT_VALUE + "='" + getDefaultDepartment(uservo).getName() + "'>").append("</" + MobileConstant.TAG_TEXTFIELD + ">");
	...
	private DepartmentVO getDefaultDepartment(BaseUser uservo) throws Exception{
		String id = uservo.getDefaultDepartment();
		DepartmentProcess process = (DepartmentProcess) ProcessFactory.createProcess(DepartmentProcess.class);
		if (!StringUtil.isBlank(id)){
			return (DepartmentVO) process.doView(id);
		}
