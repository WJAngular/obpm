...
public abstract class BaseAction<E> extends ActionSupport implements Action {
	...
	public String getApplication() {
		if(application!=null && application.indexOf(",")>-1){
	...
	protected void validateQueryParams() throws Exception {
		...
		if (content == null || content.getId() == null) {
			...
			for (Iterator<String> it = keys.iterator(); it.hasNext();) {
				...
				if (value instanceof String) {
					// 不检查Button值
					if (key.indexOf("Button") >= 0 || value.toString().indexOf("<img src=") >= 0) {
						continue;
					}

					String string = value.toString().trim();
					if (specialSymbols(string)) {
						throw new OBPMValidateException("{*[core.special.symbols.error]*}: " + value.toString()+" ,"+"{*[core.special.symbols.excel_select_error]*}");
					} else if (string.indexOf('_') >= 0) {
						// "_" 会导致查找到所有数据
						if (string.replaceAll("_", "").trim().length() == 0) {
							throw new OBPMValidateException("{*[core.special.symbols.error]*}: " + string);
						}
					}
				}
			}
	 ...
	 * @param defaultValue
	 * @return
	 * @see cn.myapps.util.property.MultiLanguageProperty#getProperty(String,
	 *      String, String)
	 * @author Tom
	 */
