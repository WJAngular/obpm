 ...
 *
 */
public class WarpURLTag extends ContextBeanTag {
    ...
    protected void populateParams() {
		...
		if(value != null && value.indexOf("/phone/resource/")>=0 && Environment.getInstance().cdnConfig.isEnable() ){//phone皮肤静态资源引用自动切换到cdn服务器
    		StringBuffer warpUrl = new StringBuffer();
    		url.setIncludeContext(false);
			warpUrl.append(Environment.getInstance().cdnConfig.getSourceHost()).append("/")
			.append(Environment.getInstance().cdnConfig.getResVersion());
    		warpUrl.append(this.value);
        	value = warpUrl.toString();
    	}
	    url.setValue(value);
    }
