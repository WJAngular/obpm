...
class BodyContentPrint implements Serializable {
	...
	private String getCurrentApplication(HttpServletRequest request) {
		String temp = request.getParameter("application");
	...
	private String getUserLanguage(HttpServletRequest request) {
		...
		String userLanguage = "CN";

		if(loc != null){
			String locLang = loc.getLanguage().toUpperCase();
			if("ZH".equals(locLang)){
				if (loc.equals(Locale.CHINA) || loc.equals(Locale.PRC))
					userLanguage = "CN";
				else if (loc.equals(Locale.TAIWAN))
					userLanguage = "TW";
				else{	//IE11
					String locVar = loc.getVariant().toUpperCase();
					if(locVar != null && !"".equals(locVar)){
						if("CN".equals(locVar)){
							userLanguage = "CN";
						}else if("TW".equals(locVar)){
							userLanguage = "TW";
						}
					}
				}
			}else if("EN".equals(locLang)){
				userLanguage = "EN";
			}
		}
