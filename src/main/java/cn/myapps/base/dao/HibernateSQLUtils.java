...
public class HibernateSQLUtils implements SQLUtils {
	...
	public String createWhere(ParamsTable params) {
		...
		while (iter.hasNext()) {
			...
			if (st > 0 && paramsValue != null && paramsValue.length() > 0) {
				...
				cndtn = cndtn.trim().endsWith("(") ? cndtn.substring(0, cndtn.trim().length()-1) : cndtn;
			}
		}
	...
	public String appendConditionToLast(String sql, String condition) {
		...
		int index;
		if ((index = end.toLowerCase().indexOf(" where ")) >= 0) {
			// 存在where
			if(condition.toUpperCase().startsWith("PARENT IN")){
				// Excel导出用到,使用or条件查询得到所有子文档
				newSQL = start + end.substring(0, index + 7) + " (" + condition + ")"; //or " + end.substring(index + 7); 2013-12-06 Excel导出子表不需要后续条件
				return newSQL;
			}
			if(newSQL.indexOf(".*") > 0 || end.toLowerCase().indexOf(" or ") >= 0){
				//sql语句定义了表别名或存在or条件
				newSQL = "SELECT * FROM(" + newSQL + ") tb_alias WHERE (" + condition + ")";
			}else{
				newSQL = start + end.substring(0, index + 7) + " (" + condition + ") and " + end.substring(index + 7);
			}
		} else if ((index = end.toLowerCase().indexOf(" order by ")) >= 0) {
			// 没有where但有order by
