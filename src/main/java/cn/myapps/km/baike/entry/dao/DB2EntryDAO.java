 ...
 * 
 */
public class DB2EntryDAO extends AbstractEntryDAO implements EntryDao {

	public DB2EntryDAO(Connection conn) throws Exception {
		super(conn);
		if (conn != null) {
			try {
...
//		log.info(sql);
	...
	public Entry findByName(String name) throws Exception {
		PreparedStatement stmt = null;
		String sql = "SELECT  * FROM "+getFullTableName("BAIKE_ENTRY") +" WHERE NAME=? FETCH FIRST 1 ROWS ONLY";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
	...
	public Collection<Entry> doFindContributer(String entryId) throws Exception {
		...
		String sql="SELECT   * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" CONTENT,"+getFullTableName("BAIKE_ENTRY")+" ENTRY WHERE ENTRY.ID =? AND ENTRY.ID=CONTENT.ENTRYID AND CONTENT.STATE='PASSED' ORDER BY HANDLETIME DESC FETCH FIRST 5 ROWS ONLY ";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
...
" ATTRIBUTE WHERE ENTRY.ID=CONTENT.ENTRYID AND ATTRIBUTE.USERID=CONTENT.AUTHOR AND ATTRIBUTE.USERID=ENTRY.AUTHOR AND CONTENT.STATE='PASSED' AND ATTRIBUTE.USERID=?";
	...
	public DataPackage<Map> getScoreboardByDepartment(int page, int lines) throws Exception {
		...
		String sql = "SELECT TD.ID, TD.NAME, SUM(TD.INTEGRAL) AS TOTALINTEGRAL FROM  " 
					...
					+ "FROM BAIKE_USER_ATTRIBUTE BAIKE, "+dabaSourceName+".DBO.T_USER,"+dabaSourceName+".DBO.T_DEPARTMENT" 
