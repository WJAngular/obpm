 ...
 * 
 */
public class OracleEntryDAO extends AbstractEntryDAO implements EntryDao {

	public OracleEntryDAO(Connection conn) throws Exception {
		super(conn);
		if (conn != null) {
			try {
...
//		log.info(sql);
	...
	public Entry findByName(String name) throws Exception {
		PreparedStatement stmt = null;
		String sql = "SELECT  * FROM "+getFullTableName("BAIKE_ENTRY") +" WHERE NAME=? AND ROWNUM <=1";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
	...
	public Collection<Entry> doFindContributer(String entryId) throws Exception {
		...
		String sql="SELECT  * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" CONTENT,"+getFullTableName("BAIKE_ENTRY")+" ENTRY WHERE ENTRY.ID =? AND ENTRY.ID=CONTENT.ENTRYID AND CONTENT.STATE='PASSED' AND ROUNUM <= 5 ORDER BY HANDLETIME DESC";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
