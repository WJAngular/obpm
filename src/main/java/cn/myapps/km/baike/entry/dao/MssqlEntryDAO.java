 ...
 * 
 */
public class MssqlEntryDAO extends AbstractEntryDAO implements EntryDao {

	public MssqlEntryDAO(Connection conn) throws Exception {
		super(conn);
		dbTag = "MS SQL SERVER: ";
		try {
...
//		log.info(sql);
	...
	public Entry findByName(String name) throws Exception {
		PreparedStatement stmt = null;
		String sql = "SELECT TOP 1 * FROM "+getFullTableName("BAIKE_ENTRY") +" WHERE NAME=?";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
	...
	public Collection<Entry> doFindContributer(String entryId) throws Exception {
		...
		String sql="SELECT TOP 5  * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" CONTENT,"+getFullTableName("BAIKE_ENTRY")+" ENTRY WHERE ENTRY.ID =? AND ENTRY.ID=CONTENT.ENTRYID AND CONTENT.STATE='PASSED' ORDER BY HANDLETIME DESC";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
