...
public class OracleEntryContentDAO extends AbstractEntryContentDAO implements EntryContentDAO{
	
	
	public OracleEntryContentDAO(Connection conn) throws Exception {
		super(conn);
		if (conn != null) {
			try {
	...
	public EntryContent getLatestVersionContent(String entryId) throws Exception {
		...
		String sql="SELECT  * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" WHERE ROWNUM < 1 AND ENTRYID =? AND STATE ='" + EntryContent.STATE_PASSED + "' ORDER BY HANDLETIME DESC";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
	...
	public EntryContent getContent(String entryId) throws Exception {
		PreparedStatement stmt = null;
		EntryContent ec = new EntryContent();
		String sql="SELECT  * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" WHERE ENTRYID =? AND ROWNUM <= 1  ORDER BY HANDLETIME DESC";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
