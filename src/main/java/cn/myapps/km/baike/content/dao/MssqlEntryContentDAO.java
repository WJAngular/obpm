...
public class MssqlEntryContentDAO extends AbstractEntryContentDAO implements EntryContentDAO{
	
	
	public MssqlEntryContentDAO(Connection conn) throws Exception {
		super(conn);
	...
	public EntryContent getLatestVersionContent(String entryId) throws Exception {
		...
		String sql="SELECT TOP 1  * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" WHERE ENTRYID =? AND STATE ='" + EntryContent.STATE_PASSED + "' ORDER BY HANDLETIME DESC";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
	...
	public EntryContent getContent(String entryId) throws Exception {
		PreparedStatement stmt = null;
		EntryContent ec = new EntryContent();
		String sql="SELECT  TOP 1 * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" WHERE ENTRYID =?  ORDER BY HANDLETIME DESC";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
