...
public class DB2EntryContentDAO extends AbstractEntryContentDAO implements EntryContentDAO{
	
	
	public DB2EntryContentDAO(Connection conn) throws Exception {
		super(conn);
		if (conn != null) {
			try {
	...
	public EntryContent getLatestVersionContent(String entryId) throws Exception {
		...
		String sql="SELECT   * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" WHERE ENTRYID =? AND STATE ='" + EntryContent.STATE_PASSED + "' ORDER BY HANDLETIME DESC FETCH FIRST 1 ROWS ONLY";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
	...
	public EntryContent getContent(String entryId) throws Exception {
		PreparedStatement stmt = null;
		EntryContent ec = new EntryContent();
		String sql="SELECT  * FROM "+getFullTableName("BAIKE_ENTRY_CONTENT")+" WHERE ENTRYID =?  ORDER BY HANDLETIME DESC FETCH FIRST 1 ROWS ONLY";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
	...
	public void removeDraft(String entryId) throws Exception{
		PreparedStatement stmt = null;		
		String sql = "DELETE " + getFullTableName("BAIKE_ENTRY_CONTENT") + " WHERE STATE='DRAFT' AND VERSIONNUM=0 AND ENTRYID=?";	
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, entryId);
