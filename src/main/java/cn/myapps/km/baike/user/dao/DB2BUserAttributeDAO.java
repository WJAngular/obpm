 ...
 * 
 */
public class DB2BUserAttributeDAO extends AbstractBUserAttributeDAO implements
		BUserAttributeDAO {
	
	public DB2BUserAttributeDAO(Connection conn) throws Exception {
		super(conn);
		if (conn != null) {
			try {
...
//	}
	...
	public void addPoint(String userId, int point) throws Exception {
		...
		String sql = "SELECT DISTINCT USERID FROM " + getFullTableName("BAIKE_USER_ATTRIBUTE")+" USERS, "+getFullTableName("BAIKE_ENTRY_CONTENT")+" CONTENT WHERE USERS.USERID=CONTENT.AUTHOR AND CONTENT.STATE='PASSED' AND ENTRYID='"+entryId+"' FETCH FIRST 5 ROWS ONLY";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			ResultSet rs = stmt.executeQuery();
