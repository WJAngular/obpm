 ...
 * 
 */
public class OracleBUserEntrySetDAO extends AbstractBUserEntrySetDAO implements
		BUserEntrySetDAO {
	public OracleBUserEntrySetDAO(Connection conn) throws Exception {
		super(conn);
		if (conn != null) {
			try {
	...
	public boolean isPublicDiskAdmin(String userId,int r_level) throws Exception {
		...
		String sql = "SELECT COUNT(*) AS ROW_COUNT FROM KM_USER_ROLE_SET A WHERE A.USERID=? AND A.ROLEID IN (SELECT ID FROM KM_ROLE B WHERE B.R_LEVEL=?)";
