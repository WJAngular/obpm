 ...
 * 
 */
public class MySqlBUserAttributeDAO extends AbstractBUserAttributeDAO implements
		BUserAttributeDAO {
	
	public MySqlBUserAttributeDAO(Connection conn) throws Exception {
		super(conn);
		dbTag = "MY SQL: ";
		try {
...
//	}
	...
	public void addPoint(String userId, int point) throws Exception {
		...
		String sql = "SELECT DISTINCT  USERID FROM " + getFullTableName("BAIKE_USER_ATTRIBUTE")+" USERS, "+getFullTableName("BAIKE_ENTRY_CONTENT")+" CONTENT WHERE USERS.USERID=CONTENT.AUTHOR AND CONTENT.STATE='PASSED' AND ENTRYID='"+entryId+"' limit 0 ,5";
		log.info(sql);
		try {
			stmt = connection.prepareStatement(sql);
			ResultSet rs = stmt.executeQuery();
