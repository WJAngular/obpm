 ...
 * 
 */
public class MssqlBUserEntrySetDAO extends AbstractBUserEntrySetDAO implements
		BUserEntrySetDAO {
	public MssqlBUserEntrySetDAO(Connection conn) throws Exception {
		super(conn);
		dbTag = "MS SQL SERVER: ";
		try {
	...
	public boolean isPublicDiskAdmin(String userId,int r_level) throws Exception {
		...
		String sql = "SELECT COUNT(*) AS ROW_COUNT FROM " +dabaSourceName+".DBO.KM_USER_ROLE_SET A WHERE A.USERID=? AND A.ROLEID IN (SELECT ID FROM "+dabaSourceName+".DBO.KM_ROLE B WHERE B.R_LEVEL=?)";
