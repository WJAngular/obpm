...
public class MySqlNDiskDAO extends AbstractNDiskDAO implements NDiskDAO {

	public MySqlNDiskDAO(Connection conn) throws Exception {
		super(conn);
		dbTag = "MY SQL: ";
		if (conn != null) {
			try {
				...
				if (this.schema.indexOf("?USE") > 0) {
					this.schema = this.schema.substring(
							this.schema.lastIndexOf("/") + 1,
							this.schema.indexOf("?USE"));
				} else {
