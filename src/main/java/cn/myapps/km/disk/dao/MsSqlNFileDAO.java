...
public class MsSqlNFileDAO extends AbstractNFileDAO implements NFileDAO {

	public MsSqlNFileDAO(Connection conn) throws Exception {
		super(conn);
		dbTag = "MS SQL SERVER: ";
		try {
	...
	protected String buildLimitString(String sql, int page, int lines, String orderbyField, String orderbyMode)
		...
		if (9 <= databaseVersion) {// 2005 row_number() over () 分页
			...
			pagingSelect.append("SELECT ROW_NUMBER() OVER (ORDER BY ").append(orderbyField).append(" ")
					.append(orderbyMode).append(") AS ROWNUMBER, TABNIC.* FROM (");
