...
public class AttachmentServlet extends HttpServlet implements Servlet {
	...
	/**所有文件的总大小*/
	public static final long ATTACHMENT_MAX_SIZE = PersonalMessageConfig.getInteger("attachment.max.size", 50 * 1000 * 1024); // 50M
	/**每次最多上传文件个数*/
	public static final long ATTACHMENT_MAX_COUNT = PersonalMessageConfig.getInteger("attachment.max.count", 3); // 5
	...
	private void processFileUpload(HttpServletRequest request,
		...
		PM_AttachmentVO attachment = new PM_AttachmentVO();
	...
	private void saveAttachment(PM_AttachmentVO attachment) throws Exception {
