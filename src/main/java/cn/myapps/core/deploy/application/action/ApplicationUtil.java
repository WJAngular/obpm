...
import cn.myapps.core.overview.OverviewBuilder;
...
public class ApplicationUtil {
		...
		if (col != null && col.size() > 0) {
			...
			while (it.hasNext()) {
				...
				if(vw.getViewType()== View.VIEW_TYPE_MAP 
						...
						|| vw.getViewType()== View.VIEW_TYPE_CALENDAR){
	...
	public Map<String, String> getRelatedFormsByModule(String moduleid, String application) throws Exception {
		LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
		map.put("none", "{*[Select]*}");
		// //PersistenceUtils.getSessionSignal().sessionSignal++;
		if (moduleid != null && !moduleid.equals("")) {
			FormProcess proc = (FormProcess) ProcessFactory.createProcess(FormProcess.class);
			Collection<Form> col = proc.getRelatedFormsByModule(moduleid, application);
	...
	//包含元素里通过软件获得首页
	public Map<String, String> getUserDefinedByApplication4Included(String application) throws Exception {
		LinkedHashMap<String, String> map = new LinkedHashMap<String, String>();
		map.put("none", "Select");
		// //PersistenceUtils.getSessionSignal().sessionSignal++;
		UserDefinedProcess pp = (UserDefinedProcess) ProcessFactory.createProcess(UserDefinedProcess.class);
		...
		while (it.hasNext()) {
			UserDefined p = it.next();
			if(p.getDefineMode() == UserDefined.CUSTOMIZE_MODE){
				map.put(p.getId(), p.getName());
			}
	...
	public Map<String, String> getImpMppingConfigByApplition(String applicationid) throws Exception {
		...
		if (list != null && list.size() > 0) {
			for (Iterator<IMPMappingConfigVO> iterator = list.iterator(); iterator.hasNext();) {
				IMPMappingConfigVO vo = (IMPMappingConfigVO) iterator.next();
				map.put(vo.getId(), vo.getName());
	...
	//包含元素里创建首页
	public String creatPage4Included(String selectFieldName, String application, String def) throws Exception {
		Map<String, String> map = getUserDefinedByApplication4Included(application);
		return DWRHtmlUtils.createOptions(map, selectFieldName, def);
	}
