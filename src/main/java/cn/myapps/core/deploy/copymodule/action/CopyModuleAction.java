package cn.myapps.core.deploy.copymodule.action;
...
import cn.myapps.core.deploy.copymodule.ejb.CopyModuleProcess;
import cn.myapps.core.deploy.copymodule.ejb.CopyModuleVO;
	...
	public String doCopyForm() throws Exception {
		try {
			...
			if (formId != null && formId[0] != null) {
				if (formId != null && formId.length > 0) {
					for (int i = 0; i < formId.length; i++) {
						if (!StringUtil.isBlank(formId[i])) {
							String _formid = Sequence.getSequence();
				...
				throw new OBPMValidateException("[{*[Errors]*}]: [{*[Not_Choose]*}]!");
			}
	...
	public String doCopyModule() throws Exception {
		try {
			...
			if (_isCopyModuel) {
				...
				vo.setModuleId(Sequence.getSequence());
			...
			if (flowid != null && flowid.length > 0) {
				...
				for (int i = 0; i < flowid.length; i++) {
					if (!StringUtil.isBlank(flowid[i])) {
						String _flowid = Sequence.getSequence();
