importPackage(Packages.cn.myapps.core.macro.runner);
importPackage(Packages.cn.myapps.util.file); // 文件工具
importPackage(Packages.cn.myapps.util.http); // Http、URL工具
importPackage(Packages.cn.myapps.util.mail); // 邮件工具

/**
 * 公共变量MAP对象
 */
var GLOBAL_MAP = Packages.cn.myapps.constans.Environment.GLOBAL_MAP;
...
function sumSubDocument(formName, fieldName) {
	...
	var accurateCalculate = $BEANFACTORY
			.createObject("cn.myapps.util.Arith");
	var total = 0;
	if (doc != null) {
		var subdocs = doc.getChilds(formName);
		if (subdocs != null && subdocs.size() > 0) {
...
function getParentDocument() {
	...
	if (parent == null && doc != null) {
		...
		if(parent == null && parentid != null) {
			parent = new Packages.cn.myapps.core.dynaform.document.ejb.Document();
 ...
 * @return 生成系统类对象
 * @param pathText
 *            类的名字空间,如"cn.myapps.util.DateUtil";
 * 
 */
 ...
 * @return 生成系统类对象
 * @param pathText
 *            类的名字空间,如"cn.myapps.core.department.ejb.DepartmentProcess";
 * 
 */
...
function getDocProcess(applicationid) {
	return Packages.cn.myapps.core.dynaform.document.ejb.DocumentProcessBean
...
function getUploadProcess(applicationid) {
	return Packages.cn.myapps.core.upload.ejb.UploadProcessBean
			.createMonitoProcess(applicationid);
}
...
function createParamsTable() {
	return new Packages.cn.myapps.base.action.ParamsTable();
}
...
function sendMessage(senderid, receiverid, title, content) {
	var process = createProcess("cn.myapps.core.personalmessage.ejb.PersonalMessageProcess");
 ...
 *            信息類型
 */
function sendMessageWithType(senderid, receiverid, title, content,type) {
	var process = createProcess("cn.myapps.core.personalmessage.ejb.PersonalMessageProcess");
 ...
 *            企业域id          
 */
function sendSMS4Task(title, content, receiver, isReply, isMass,applicationId,domainId){
	var user = new Packages.cn.myapps.core.user.action.WebUser(null);
	user.setDomainid(domainId);
	user.setDefaultApplication(applicationId);
	var sender = $MESSAGE.getSMSManager().getSender(user);
	return sender.send("", title, content, receiver, isReply, isMass);
}
 ...
 *            是否校验
 */
function sendMailtoAllUser(from, subject, content, host, account, password, bbc, validate) {
	$EMAIL.sendMailToAllUser(from, subject, content, host, account, password, bbc,
			validate);
}
 ...
 *            内容
 * @param attachFiles
 *            附件列表（数据格式：字符串数组；内容：存放附件的完整物理地址如“D:\\附件.txt”）
 * @param host
 *            邮件服务器地址
 * @param user
 *            邮件服务器用户名
 ...
 *            是否校验
 */
function sendMailWithAttachments(from, to, subject, body,attachFiles, host, user, password, bbc, validate) {
	$EMAIL.setEmail(from, to, subject, body,attachFiles, host, user, password, bbc,
			validate);
	$EMAIL.send();
}
 ...
 *            是否校验
 */
function sendMailWithAttachmentsToAllUser(from, subject, content,attachFiles, host, account, password, bbc, validate) {
	$EMAIL.sendMailToAllUser(from, subject, content,attachFiles, host, account, password, bbc,
			validate);
}
/**
 * 以系统配置的用户发送邮件(带附件)
 ...
 * @return
 */
function sendEmailWithAttachmentsBySystemUser(to, subject, content,attachFiles) {
	$EMAIL.sendEmailBySystemUser(to, subject, content,attachFiles);
}
...
function getAllRoles() {
	var roleProcess = createProcess("cn.myapps.core.role.ejb.RoleProcess");
...
function getDataSourceProcess() {
	return createProcess("cn.myapps.core.dynaform.dts.datasource.ejb.DataSourceProcess");
}
...
function getUserProcess() {
	return createProcess("cn.myapps.core.user.ejb.UserProcess");
}
...
function getDepartmentProcess() {
	return createProcess("cn.myapps.core.department.ejb.DepartmentProcess");
}
...
function getRoleProcess() {
	return createProcess("cn.myapps.core.role.ejb.RoleProcess");
}
/**
 * 返回表单业务对象
 * 
...
function getFormProcess() {
	return createProcess("cn.myapps.core.dynaform.form.ejb.FormProcess");
}
/**
 * 返回视图业务对象
 * 
...
function getViewProcess() {
	return createProcess("cn.myapps.core.dynaform.view.ejb.ViewProcess");
}
...
function getDomainProcess() {
	return createProcess("cn.myapps.core.domain.ejb.DomainProcess");
}
...
function getLastRelationHis(docid, flowid){
	var process = new Packages.cn.myapps.core.workflow.storage.runtime.ejb.RelationHISProcessBean(getApplication());
...
function getRoleByRoleNo(roleno, applicationid){
	var process = new Packages.cn.myapps.core.role.ejb.RoleProcessBean();
 ...
 * @return
 */
function closeSessionAndConnection(){
	$TOOLS.PERSISTENCE_UTIL.closeSessionAndConnection();
}
 ...
 * 		true or false
 */
function isWorkingTime(currentDate){
	var user = $WEB.getWebUser();
	return isWorkingTimeByCalendar(currentDate, user.getCalendarType());
}
 ...
 * 		true or false
 */
function isWorkingTimeByCalendar(currentDate, calendarId){
	var bool = $TOOLS.DATE_UTIL.isWorkingTimeByCalendar(currentDate, calendarId);
	return bool;
}
 ...
 * 		返回用户id的集合
 */
function getSelectUsers4SubFlowNode(subflowNodeId) {
	var result = new Packages.java.util.ArrayList();
	var _subFlowApproverInfo=getParameter("_subFlowApproverInfo");
	if(_subFlowApproverInfo != null && _subFlowApproverInfo.trim().length()>0){
		var jsonUtil= new Packages.cn.myapps.util.json.JsonUtil();
		var subFlowApproverInfos=jsonUtil.toCollection(_subFlowApproverInfo.trim());
		for(var iter=subFlowApproverInfos.iterator();iter.hasNext();) {
			var subFlowApproverInfo=iter.next();
			var _nodeId=subFlowApproverInfo.get("nodeid");
			if(subflowNodeId !=null && subflowNodeId.length>0){
				if(_nodeId != subflowNodeId) continue;
			}
			var approvers=subFlowApproverInfo.get("approver");
			for(var i=0;i<approvers.length;i++){
				var approver=approvers[i];
				var userids= approver.get("userids");
				for (var j = 0; j <userids.length; j++) {
					result.add(userids[j]);
				}
			}
		}
	}
 ...
 * 	返回用户id的集合
 */
function getSelectUsers4ManualNode(manualNodeId) {
	var result = new Packages.java.util.ArrayList();
	var submitTo=getParameter("submitTo");
	if(submitTo != null && submitTo.trim().length()>0){
		var jsonUtil= new Packages.cn.myapps.util.json.JsonUtil();
		var submitInfo = jsonUtil.toCollection(submitTo);
		for (var iterator = submitInfo.iterator(); iterator.hasNext();) {
			var tmpmap = iterator.next();
			var _nodeId = tmpmap.get("nodeid");
			if(manualNodeId !=null && manualNodeId.length>0){
				if(_nodeId != manualNodeId) continue;
			}
			var selectuserlist = jsonUtil.toCollection(tmpmap.get("userids"));
			result.addAll(selectuserlist);
		}
	}
 ...
 * 	返回服务器地址
 */
function getServerHostAddr(){
	var request = $WEB.getParamsTable().getHttpRequest();
	if (request == null){
		return null;
	}
	var addr = request.getScheme()+"://"+request.getServerName();
	if(request.getServerPort()!=80){
		addr+=":"+request.getServerPort();
	}
	if(request.getContextPath().length()>0){
		addr+=request.getContextPath();
	}
	return addr;
 ...
 * @throws Exception
 */
function sendWeixinRichTextMessage(touser,title,description,url,picUrl,domainId,applicationid){
	return Packages.cn.myapps.support.weixin.WeixinServiceProxy.sendRichTextMessage(touser,title,description,url,picUrl,domainId,applicationid);
}
/**
 * 发送微信文本消息
 * @param touser
 ...
 * @throws Exception
 */
function sendWeixinTextMessage(touser,content,domainId,applicationid){
	return Packages.cn.myapps.support.weixin.WeixinServiceProxy.sendTextMessage(touser,content,domainId,applicationid);
}
/**
* 发送微信消息
* @param json
* 		微信消息(JSON格式 参考微信平台API文档:http://qydev.weixin.qq.com/wiki/index.php?title=%E6%B6%88%E6%81%AF%E7%B1%BB%E5%9E%8B%E5%8F%8A%E6%95%B0%E6%8D%AE%E6%A0%BC%E5%BC%8F)
* @param domainId
* 		企业域id
* @return
* @throws Exception
*/
function sendWeixinMessage(json,domainId){
	return Packages.cn.myapps.support.weixin.WeixinServiceProxy.sendMessage(json,domainId);
}
/**
 * 新建问卷问题
 * @param id
 ...
 * @param topic
 * 		标题
 * @returns {Packages.cn.myapps.core.dynaform.form.ejb.Question}问题对象
 */
function createQuestion(id,topic) {
	return new Packages.cn.myapps.core.dynaform.form.ejb.Question(id, topic);
}
 ...
 *
 */
function getCurrentProcessingNode(){
	var request = $WEB.getParamsTable().getHttpRequest();
	if (request != null) {
		return node = request.getAttribute("_targetNodeRT");
	}
	return null;
}
 ...
 * @param flowId
 * 		流程id
 * @returns {Packages.cn.myapps.core.workflow.element.Node}问题对象
 */
