...
import cn.myapps.core.macro.repository.ejb.SystemBaseLib;
...
public class RepositoryAction extends BaseAction<RepositoryVO> {
			...
			while (true) {
				...
				if (indexS > -1 && indexE > -1 && indexE > indexS) {
					...
					if (functionName.matches("(\\S|\\d)+")) {
						...
						String dql = "from cn.myapps.core.macro.repository.ejb.RepositoryVO rvo where rvo.applicationid='"
								+ repository.getApplicationid()
						...
						int size = ((RepositoryProcess) process)
								.doGetTotalLines(dql);
	...
	public Map<String, String> get_applications() throws Exception {
		...
		for (Iterator<ApplicationVO> ite = applications.iterator(); ite
				.hasNext();) {
			ApplicationVO application = (ApplicationVO) ite.next();
