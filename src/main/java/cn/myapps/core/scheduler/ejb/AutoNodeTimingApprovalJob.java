 ...
 *
 */
public class AutoNodeTimingApprovalJob implements Job {
	...
	public AutoNodeTimingApprovalJob(String docId, String flowStateId,
	...
	public void execute() throws Exception {
		try {
			...
			if(nodert !=null){
				...
				if(doc ==null) return;
				if(!StringUtil.isBlank(this.flowTicket)){
					doc.setFlowTicket(this.flowTicket);
				}
				...
				if(StringUtil.isBlank(actorId)){
					...
					user = new WebUser(superUserProcess.getDefaultAdmin()); // 系统用户
					user.setDomainid(instance.getDocument().getDomainid());
					user.setName("system");
				}else{
				...
				AutoNode currNode = (AutoNode) flowVO.findNodeById(nodert.getNodeid());
				...
				if (currNode != null && nextNodeList != null
					...
					instanceProcess.doApprove(new ParamsTable(), instance, nodert.getNodeid(),
							nextids, FlowType.AUTO2RUNNING, "", user);
