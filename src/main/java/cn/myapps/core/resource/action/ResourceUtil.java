...
/*	*//**
	...
	public String getMenuTree(String appid, String domainid, String basePath,
		...
		for (Iterator<ResourceVO> iter = topMenus.iterator(); iter.hasNext(); i++) {
			ResourceVO resource = (ResourceVO) iter.next();
...
//			}
			...
			dm.put("", "{*[cn.myapps.core.resource.none]*}");
		}
	...
	public boolean isShowTotalRow(ResourceVO resource){
		try {
			if(resource!=null){
				if(isLinkToView(resource.getId())){
	...
	public String getTotalRowByResourceid(String resourceid,HttpServletRequest request){
		...
		try {
			...
			if(resource.getLinkType().equals(ResourceVO.LinkType.VIEW.getCode())){
				...
				View view = (View) viewprocess.doView((String)resource.getActionContent());
				if (view != null) {
	...
	public Collection<ResourceVO> getResourcesByApp(String application) throws Exception{
		...
		Collection<ResourceVO> cols=(Collection<ResourceVO>)resourceProcess.doQuery(params).datas;
		return cols;
	}
}