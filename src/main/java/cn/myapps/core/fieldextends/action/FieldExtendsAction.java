package cn.myapps.core.fieldextends.action;
...
public class FieldExtendsAction extends BaseAction<FieldExtendsVO> {
		...
		try {
			...
			if (StringUtil.isBlank(fieldExtends.getFid())) {
				...
				if(fieldExtendses != null && fieldExtendses.size() > 0){
					...
					((FieldExtendsProcess) process).doUpdate(oldField);
			...
			addFieldError("addError", "{*[Save]*}{*[field.extends.message.failed]*}"+e.getValidateMessage());//field.extends.message.addError
			return INPUT;
		}catch (Exception e) {
			this.setRuntimeException(new OBPMRuntimeException("{*[Save]*}{*[field.extends.message.failed]*}:"+"{*[OBPMRuntimeException]*}",e));
			e.printStackTrace();
			return INPUT;
		}
	}
	...
	public String doList() {
		try {
			...
			addFieldError("showFieldExtendsList", e.getValidateMessage());
			return INPUT;
		}catch (Exception e) {
	...
	public String doDelete() {
		if (fieldNameAndIds == null) {
			addFieldError("deleteError", "{*[page.records.notChoose]*}");	
			return doList();
		}
		try {
			//查询结果
			...
			addFieldError("deleteError", "{*[field.extends.message.deleteError]*}"+e.getValidateMessage());	
			return doList();
		}catch (Exception e) {
			this.setRuntimeException(new OBPMRuntimeException("{*[OBPMRuntimeException]*}",e));
			e.printStackTrace();
