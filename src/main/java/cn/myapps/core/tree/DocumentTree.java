...
public class DocumentTree extends Tree<Document> {
		...
		if (docs != null && !docs.isEmpty()) {
			try {
				...
				for (Iterator<Document> iterator = docs.iterator(); iterator
					...
					String value = doc
							.getValueByField(((Column) columnMapFields
	...
	public void search() {
		try {
			Map<String, Column> columnMapFields = view.getViewTypeImpl()
					.getColumnMapping();
			String nodeNameField = ((Column) columnMapFields
	...
	public void deepSearchParent(Document child) throws Exception {
		...
		Map<String, Column> columnMapFields = view.getViewTypeImpl()
				.getColumnMapping();
		String relationField = ((Column) columnMapFields
				.get(TreeType.DEFAULT_KEY_FIELDS[0])).getFieldName(); // 上级树节点
		String valueField = ((Column) columnMapFields
	...
	protected Node createNode(Document doc) throws Exception {
		...
		while (it.hasNext()) {
			...
			textsMap.put(key.getId(), value.toString());
		}
		...
		Map<String, Column> columnMapFields = view.getViewTypeImpl()
				.getColumnMapping();
		// 当前树节点名称
		String text = textsMap.get(((Column) columnMapFields
				.get(TreeType.DEFAULT_KEY_FIELDS[2])).getId());
		// 当前树节点(ID)
		String value = doc.getValueByField(((Column) columnMapFields
