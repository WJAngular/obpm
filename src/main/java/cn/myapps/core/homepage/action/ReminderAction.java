...
//			}
			...
			this.setRuntimeException(new OBPMRuntimeException( "{*[Save]*}{*[Error]*}:"+"{*[OBPMRuntimeException]*}",e));
			e.printStackTrace();
			return INPUT;
		}
	...
	public String confirm() throws Exception {
		try {
			...
			if (ids == null || ids.length < 0) {
				throw new OBPMValidateException("{*[page.records.notChoose]*}");
			} else {
				String homepageid = params.getParameterAsString("s_homepage");
				HomePageProcess process = getProcess();
				HomePage homepage = (HomePage) process.doView(homepageid);
