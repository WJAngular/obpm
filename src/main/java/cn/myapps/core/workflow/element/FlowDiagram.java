...
import cn.myapps.core.workflow.engine.RunActionException;
...
public class FlowDiagram extends Canvas {
	...
	public void editManualNode(ManualNode grp, String id, String name, String statelabel, String namelist, String note,
			String passcondition, String exceedaction, String backnodeid, String formname, String fieldpermlist,String activityPermList, boolean issplit,
		...
		grp.activityPermList = activityPermList;
		grp.issplit = issplit;
	...
	public ManualNode addManualNode(String name, String statelabel, int x, int y) {
		...
		editManualNode(grp, id, name, statelabel, namelist, note, passcondition, exceedaction, backnodeid, formname,
				fieldpermlist,activityPermList, issplit, isgather, actorEditMode, actorListScript, notificationStrategyJSON, cBack, isToPerson,
	...
	public void paintMobile(OGraphics mg) {
		...
		// this.setSize(width, height);
		mg.setColor(this.getBackground());

		// 画背景画布
		double rate = mg.getCompressRate();
		mg.setCompressRate(1.0);
		mg.fillRect(0, 0, 2048, 1536);
	...
	public Node getFirstCurrentNode() {
		...
		while (enum11.hasMoreElements()) {
			Element item = (Element) enum11.nextElement();
			if (item instanceof Node) {
				Node nd = (Node) item;
				if (nd._iscurrent) {
					return nd;
		...
		while (enum11.hasMoreElements()) {
			...
			if (item instanceof Node) {
				...
				for (Iterator<Element> iterator = subElements.iterator(); iterator.hasNext();) {
					Object subElment = (Object) iterator.next();
	 ...
	 * @throws Exception
	 */
	public Node getNextNode(Relation r,Document doc,ParamsTable params,WebUser user) {
		Node end = null;
		...
		try {
			String labelId = getId() + "-"+r.id+ "-" + r.startnodeid + "-" + r.endnodeid;
	...
	public void runAction(IRunner runner, String startnodeid, String endnodeid) throws RunActionException, Exception {
		Relation relation = this.getRelation(startnodeid, endnodeid);
		if (relation != null) {
			...
			if (action != null && action.trim().length() > 0) {
				...
				if (rtn instanceof String && !StringUtil.isBlank((String) rtn)) {
					throw new OBPMValidateException((String) rtn,new RunActionException((String) rtn));
				}
			}
		}
	}
	...
	public Vector<Node> getBackSetpNode(Node node) {
		...
		for (Iterator<Relation> iter = allrelation.iterator(); iter.hasNext();) {
			Relation re = (Relation) iter.next();
	 ...
	 * @return
	 */
	public Vector<Node> getBackSetpElement(Node node) {
		Vector<Node> nodes = new Vector<Node>();
		Vector<Relation> allrelation = getNodeBackStepRelation(node);
		for (Iterator<Relation> iter = allrelation.iterator(); iter.hasNext();) {
			Relation re = (Relation) iter.next();
	...
	public Vector<Relation> getNodeBackStepRelation(Node nd) {
		...
		for (Iterator<Element> iter = _elems.iterator(); iter.hasNext();) {
			Element subem = (Element) iter.next();
			if (subem instanceof Relation) {
				Relation relation = (Relation) subem;
				if (relation.id != null && relation.endnodeid.equals(nd.id)) {
					rv.add(relation);
	...
	public Collection<Node> getNextNodeList(String currnodeid,Document doc,ParamsTable params,WebUser user) {
		...
		if (em instanceof Node) {
			...
			if (relations != null) {
				...
				while (it.hasNext()) {
					Node nextNode = this.getNextNode((Relation) it.next(),doc,params,user);
					if (nextNode != null) {
						colls.add(nextNode);
					}
				}
	 ...
	 * 获取所有后续节点列表
	 */
	public Collection<Node> getAllFollowNodeOnPath(String nodeid,final Document doc,final ParamsTable params,final WebUser user) {
		class FollowNodeOnPath {
			Collection<Node> list = new ArrayList<Node>();

			Collection<Node> getAllFollowNodeOnPath(Node beginNode) {
				Collection<Node> nextNodeList = getNextNodeList(beginNode.id,doc,params,user);
	 ...
	 * 获取当前节点列表关联的所有下一个节点
	 */
	public Collection<Node> getNextNodeList(Collection<Node> nodelist,Document doc,ParamsTable params,WebUser user) {
		ArrayList<Node> rtn = new ArrayList<Node>();
		if (nodelist != null) {
			for (Iterator<Node> iter = nodelist.iterator(); iter.hasNext();) {
				Node nd = (Node) iter.next();
				String currid = nd.id;
				Collection<Node> colls = getNextNodeList(currid,doc,params,user);
				rtn.addAll(colls);
			}
		}
		return rtn;
	...
	public Collection<SubFlow> getSubFlowNodeList() {
		...
		try {
			for (Iterator<Element> iterator = elements.iterator(); iterator.hasNext();) {
				Element elemnt = (Element) iterator.next();
