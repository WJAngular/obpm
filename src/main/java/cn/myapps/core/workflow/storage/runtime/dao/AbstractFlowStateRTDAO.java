...
public abstract class AbstractFlowStateRTDAO {
	...
	public void create(ValueObject vo, AbstractNodeRTDAO nodeRTDAO)
		...
		String sql = "INSERT INTO "
				...
				+ "(ID,FLOWID,DOCID,STATE,APPLICATIONID,PARENT,FLOWXML,FLOWNAME,LASTMODIFIERID,LASTMODIFIED,SUBFLOWNODEID,COMPLETE,CALLBACK,TOKEN,STATELABEL,INITIATOR,AUDITUSER,AUDITORNAMES,AUDITORLIST,LASTFLOWOPERATION,AUDITDATE,SUB_POSITION,ISARCHIVED,ISTERMINATED,PREV_AUDIT_NODE,PREV_AUDIT_USER)";
		sql += " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		log.info(dbTag + sql);
		try {
			if (flowstatert != null) {
				...
				setValues(statement, ++index, new Timestamp(flowstatert.getLastModified().getTime()));
				...
				setValues(statement, ++index, new Timestamp(flowstatert.getAuditdate().getTime()));
				setValues(statement, ++index, flowstatert.getPosition());
				if (flowstatert.isArchived()) {
					setValues(statement, ++index, 1);
				} else {
					setValues(statement, ++index, 0);
				}
				if (flowstatert.isTerminated()) {
					setValues(statement, ++index, Integer.valueOf(1));
				} else {
					setValues(statement, ++index, Integer.valueOf(0));
				}
				setValues(statement, ++index, flowstatert.getPrevAuditNode());
				setValues(statement, ++index, flowstatert.getPrevAuditUser());
	...
	public void update(ValueObject vo, AbstractNodeRTDAO nodeRTDAO)
		...
		String sql = "UPDATE "
				+ getFullTableName("T_FLOWSTATERT")
				+ " SET ID=?,FLOWID=?,DOCID=?,STATE=?,APPLICATIONID=?,PARENT=?,FLOWXML=?,FLOWNAME=?,LASTMODIFIERID=?,LASTMODIFIED=?,SUBFLOWNODEID=?,COMPLETE=?,CALLBACK=?,TOKEN=?,STATELABEL=?,INITIATOR=?,AUDITUSER=?,AUDITORNAMES=?,AUDITORLIST=?,LASTFLOWOPERATION=?,AUDITDATE=?,SUB_POSITION=?,ISARCHIVED=?,ISTERMINATED=?,PREV_AUDIT_NODE=?,PREV_AUDIT_USER=?";
		sql += " WHERE ID=?";
		...
		try {
			if (flowstatert != null) {
				...
				setValues(statement, ++index, flowstatert.getAuditorList());
				setValues(statement, ++index, flowstatert.getLastFlowOperation());
				if(flowstatert.getAuditdate() == null){//兼容旧数据
					flowstatert.setAuditdate(new java.util.Date());
				}
				setValues(statement, ++index, new Timestamp(flowstatert.getAuditdate().getTime()));
				setValues(statement, ++index, flowstatert.getPosition());
				if (flowstatert.isArchived()) {
	...
	private void setBaseProperties(FlowStateRT vo, ResultSet rs)
		...
		vo.setLastModified(rs.getTimestamp("LASTMODIFIED"));
		...
		if (rs.getInt("ISARCHIVED") == 1) {
			vo.setArchived(true);
		} else {
			vo.setArchived(false);
		}
		vo.setTerminated(rs.getInt("ISTERMINATED") == 1 ? true : false);
		vo.setPrevAuditNode(rs.getString("PREV_AUDIT_NODE"));
		vo.setPrevAuditUser(rs.getString("PREV_AUDIT_USER"));
	}
	...
	public FlowStateRT getCurrFlowStateRT(Document doc, WebUser user,
		...
		if (!StringUtil.isBlank(currFlowStateId)) {
			sql = "SELECT * FROM "+ getFullTableName("T_FLOWSTATERT")+" WHERE ID='" + currFlowStateId + "' ";
		}else{
			String w = "";
			if (user != null) {
				w += " AND actor.ACTORID = '" + user.getId() + "' ";
			}
			sql = "SELECT vo.ID,vo.FLOWID,vo.DOCID,vo.STATE,vo.APPLICATIONID,vo.PARENT,vo.FLOWXML,vo.FLOWNAME,vo.LASTMODIFIERID,vo.LASTMODIFIED,vo.SUBFLOWNODEID,vo.COMPLETE,vo.CALLBACK,vo.TOKEN,vo.STATELABEL,vo.INITIATOR,vo.AUDITUSER,vo.AUDITORNAMES,vo.AUDITORLIST,vo.LASTFLOWOPERATION,vo.AUDITDATE,vo.SUB_POSITION,vo.PREV_AUDIT_NODE,vo.PREV_AUDIT_USER,vo.ISARCHIVED,vo.ISTERMINATED "
					+ "FROM "
					+ getFullTableName("T_FLOWSTATERT")
					+ " vo, "
...
//			nodeRTDAO.removeByForeignKey("FLOWSTATERT_ID", id);
	...
	public boolean isAllFlowInstancesCompleted(String docId) throws Exception{
		...
		String sql = "SELECT COUNT(0) FROM " + getFullTableName("T_FLOWSTATERT")
		+ " vo WHERE vo.DOCID='"+docId+"' AND vo.COMPLETE=0 ";
	...
	public FlowStateRT getParentFlowStateRT(Document doc) throws Exception{
		...
		String sql = "SELECT * FROM "
			+ getFullTableName("T_FLOWSTATERT")
			+ " vo WHERE vo.PARENT IS NULL AND vo.DOCID=?";
		log.info(dbTag + sql);
		try {
			statement = connection.prepareStatement(sql);
