...
public abstract class AbstractCirculatorDAO {
	...
	protected String createPendingSQL(ParamsTable params, WebUser user)
		...
		sqlBuilder
				.append("SELECT c.ID ,c.NAME ,c.USERID ,c.DOC_ID ,c.NODERT_ID ,c.FLOWSTATERT_ID ,c.CCTIME ,c.READTIME ,c.DEADLINE ,c.ISREAD ,c.DOMAINID ,c.APPLICATIONID,p.SUMMARY,p.FORMID FROM "
						...
						+ " p WHERE c.DOC_ID = p.DOCID AND c.ISREAD = 0 AND p.FORMID = '"
						+ formId + "'");
		sqlBuilder.append(" AND c.USERID ='" + user.getId() + "'");
	...
	protected String createWorkSQL(ParamsTable params, WebUser user)
		...
		String orderName="CCTIME";
		if(isRead.equals("1")){
			orderName="READTIME";
		}
		String _subject4Circulator = params.getParameterAsString("_subject4Circulator");
		sqlBuilder
				.append("SELECT c.ID ,c.NAME ,c.USERID ,c.DOC_ID ,c.NODERT_ID ,c.FLOWSTATERT_ID ,c.CCTIME ,c.READTIME ,c.DEADLINE ,c.ISREAD ,c.DOMAINID ,c.APPLICATIONID,p.SUMMARY,p.FORMID FROM "
						+ getFullTableName("T_CIRCULATOR")
						+ " c,"
						+ getFullTableName("t_flow_intervention")
						+ " p WHERE c.DOC_ID = p.DOCID AND c.ISREAD = "+isRead);
		sqlBuilder.append(" AND c.USERID ='" + user.getId() + "'");
		
		sqlBuilder.append(getWorkSQLCondition(_flowId, _formId,_subject4Circulator));
		
		String rtn = this.addOrderBy(sqlBuilder.toString(), orderName);
