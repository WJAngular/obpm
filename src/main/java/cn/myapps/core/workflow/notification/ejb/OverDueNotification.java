...
public class OverDueNotification extends Notification {
	...
	protected int editMode;//编辑模式
	
	protected String limittimeScript;//过期时间脚本
	
	protected ActorRT actorrt;
		...
		if (responsibles != null && !responsibles.isEmpty()) {
			for (Iterator<BaseUser> iterator = responsibles.iterator(); iterator
				...
				if (responsible != null
					...
					ActorRTProcess actorRTPorcess = new ActorRTProcessBean(applicationid);
					this.getActorrt().setDeadline(null);
					actorRTPorcess.doUpdate(this.getActorrt());
				}
			}
		}
	}
	...
	public String getLimittimeScript() {
		return limittimeScript;
	}

	public void setLimittimeScript(String limittimeScript) {
		this.limittimeScript = limittimeScript;
	}

	public ActorRT getActorrt() {
		return actorrt;
	}

	public void setActorrt(ActorRT actorrt) {
		this.actorrt = actorrt;
	}
