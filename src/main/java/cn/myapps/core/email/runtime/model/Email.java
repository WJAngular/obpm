package cn.myapps.core.email.runtime.model;
...
//	}
	...
	public String getSubject() {
		String subject = cn.myapps.core.email.util.Utility.doCharsetCorrections(getBaseHeader().getSubject());
	...
	public Message toMessage(Session session) throws MessagingException, IOException {
		...
		if (parts.size() == 1 && isTextBody) {
			...
			for (int i=1; i < this.getParts().size(); i++) {
				myPart = (EmailPart)this.getParts().get(i);
