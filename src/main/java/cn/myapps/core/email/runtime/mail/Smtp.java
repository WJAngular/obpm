...
import cn.myapps.core.email.runtime.model.ByteArrayDataSource;
import cn.myapps.core.email.runtime.model.Email;
import cn.myapps.core.email.runtime.model.EmailPart;
import cn.myapps.core.email.runtime.model.EmailPriority;
import cn.myapps.core.email.runtime.model.EmailSensitivity;
import cn.myapps.core.email.util.EmailConfig;
import cn.myapps.util.mail.OBPMAuthenticator;
...
public class Smtp {
	...
	public Smtp(ConnectionProfile profile, AuthProfile auth) {
		...
		if (profile.getSmtpSSL() != null && profile.getSmtpSSL().toLowerCase().equals("true")) {
			...
			props.setProperty("mail.smtps.socketFactory.class", "cn.myapps.core.email.runtime.mail.OBPMSSLSocketFactory");
		...
		if (profile.getSmtpAuthenticated() != null && profile.getSmtpAuthenticated().equals("true")) {
			...
			OBPMAuthenticator authenticator = new OBPMAuthenticator(auth.getUserName() + "@" + EmailConfig.getEmailDomain(), auth.getPassword());
	...
	public HashMap sendEmail(Email msg, boolean simulate) throws Exception {
		...
		//mimeMsg.addHeader("X-Mailer", "teemlink.com (http://www.teemlink.com)");
		mimeMsg.saveChanges();
