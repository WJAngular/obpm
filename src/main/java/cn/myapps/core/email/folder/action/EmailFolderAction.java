package cn.myapps.core.email.folder.action;
...
public class EmailFolderAction extends BaseAction<EmailFolder> {
	...
	public String doSave() {
		try {
			...
			if (folder == null || StringUtil.isBlank(folder.getName())) {
				throw new OBPMValidateException(getMultiLanguage("core.email.folder.cannot.null"));
			}
			...
			if (temp != null && !temp.getId().equals(folder.getId())) {
				throw new OBPMValidateException(getMultiLanguage("core.email.folder.created"));
			}
			...
			ResponseUtil.setTextToResponse(ServletActionContext.getResponse(), "ERROR::" + e.getValidateMessage());
			return null;
		}catch (Exception e) {
	...
	public EmailUser getEmailUser() throws Exception {
		...
		if (emailUser == null) {
			throw new OBPMValidateException(getMultiLanguage("core.email.user.error"));
		}
		return emailUser;
	}
