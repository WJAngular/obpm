...
import cn.myapps.core.email.email.dao.EmailDAO;
...
public class EmailProcessBean extends AbstractDesignTimeProcessBean<Email> implements
	...
	public void doToRecy(String[] ids) throws Exception {
		...
		if (recyFolder == null) {
			// 邮件系统还没初始化
			throw new OBPMValidateException("E-mail system not initialized!");
		}
		for (int i = 0; i < ids.length; i++) {
			Email email = (Email) doView(ids[i]);
	...
	private void sendEmailToUser(Email email, EmailUser sendUser) throws Exception {
		...
		for (Iterator<String> it = email.getReceiver().iterator(); it.hasNext(); ) {
			...
			if (user == null) {
				throw new OBPMValidateException("您发送的内部邮件找不到收件人!");
			}
	...
	public void doRemoveEmailByEmailUser(String emailUserId) throws Exception{
		((EmailDAO)getDAO()).doRemoveEmailByEmailUser(emailUserId);
	}
}
