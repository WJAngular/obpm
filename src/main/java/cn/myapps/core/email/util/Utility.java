	...
	public static String[] addressArrToStringArr(Address[] addr) {
		if (addr != null && addr.length > 0) {
			...
			for (int j = 0; j < addr.length; j++) {
				...
				String personal = cn.myapps.core.email.util.Utility.doCharsetCorrections(add.getPersonal());
				String address = cn.myapps.core.email.util.Utility.doCharsetCorrections(add.getAddress());
	...
	public static Address[] stringToAddressArray(String str) throws Exception {
		...
		if (token.countTokens() != 0) {
			...
			while (token.hasMoreTokens()) {
				...
				try {
					if ((j = addr.indexOf("<")) > 0) {
						fullname = cn.myapps.core.email.util.Utility.extendedTrim(addr.substring(0, j).trim(), "\"");
						email = cn.myapps.core.email.util.Utility.extendedTrim(cn.myapps.core.email.util.Utility.extendedTrim(addr.substring(j + 1), ">"), "\"").trim();
