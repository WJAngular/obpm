...
import cn.myapps.init.InitializationException;
...
public class CalendarHelper extends BaseHelper<CalendarVO> {
	...
	public Map<String, String> getWorkCalendars() throws Exception {
		...
		if (list != null) {
			...
			while (it.hasNext()) {
				CalendarVO cld = (CalendarVO) it.next();
	...
	public String getCalendarById(String id) throws Exception {
		CalendarProcess calendarProcess = ((CalendarProcess) process);
		if (calendarProcess != null) {
			CalendarVO vo = (CalendarVO) calendarProcess.doView(id);
			if (vo != null)
				return vo.getName();
		}
		return "";
			...
			// 标准日历
			{
				try {
					...
					for (int i = 0; i < 7; i++) {
						...
						if (i == 0 || i == 6) {
							standard[i].setId(Sequence.getSequence());
	...
	public void removeCalendarByDomain(String domainid) throws Exception {
		...
		if (canlendarProcess != null) {
			...
			if (cols != null) {
				...
				while (it.hasNext()) {
					CalendarVO vo = (CalendarVO) it.next();
					if (vo != null) {
