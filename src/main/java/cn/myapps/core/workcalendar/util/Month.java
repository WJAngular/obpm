package cn.myapps.core.workcalendar.util;
...
public class Month implements java.io.Serializable {
			...
			for (int j = 0; standingDays[i] != null && j < standingDays[i].length; j++) {
				...
				if (day < start)
					...
					for (int k = 1; k <= 5; k++) {
						...
						try {
							// 设置开始时间
							Method method0 = BaseDay.class.getMethod("getStartTime" + k, new Class<?>[]{});
							String starttime = (String) method0.invoke(standardDay, new Object[]{});
							int[] fromTime = getTime(starttime);

							// 设置结束时间
							Method method1 = BaseDay.class.getMethod("getEndTime" + k, new Class<?>[]{});
							String endtime = (String) method1.invoke(standardDay, new Object[]{});
					...
					return (DayPart[]) dayPartList.toArray(new DayPart[dayPartList.size()]);
				}
			}
	...
	public double countRemainingTimes(Date crrDate) throws Exception {
		...
		for (int k = 1; k < 6; k++) {
			try {
				Method method0 = BaseDay.class.getMethod("getStartTime" + k, new Class<?>[]{});
				String starttime = (String) method0.invoke(vo, new Object[]{});
				int[] fromTime = getTime(starttime);
				if (fromTime != null)
					strtime[k - 1] = fromTime[0] + fromTime[1] / 60.0;
				Method method1 = BaseDay.class.getMethod("getEndTime" + k, new Class<?>[]{});
				String edtime = (String) method1.invoke(vo, new Object[]{});
