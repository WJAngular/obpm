...
//				+ recvtel + "%') or (mass=1)) and submission=false and sendDate >=?";
	...
	public DataPackage<SubmitMessageVO> list(WebUser user, ParamsTable params) throws Exception {
		...
		String hql = " from " + _voClazzName + " vo where ";
		hql += " vo.sender='"+tel+"'";
		hql += " and receiverName like '%" + StringUtils.defaultString(params.getParameterAsString("_receiverName"),"") + "%'";
		hql += " and content like '%" + StringUtils.defaultString(params.getParameterAsString("_content"),"") + "%'";
		hql += " order by vo.sendDate desc";
