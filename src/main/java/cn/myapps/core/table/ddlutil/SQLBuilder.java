...
public abstract class SQLBuilder {
	...
	protected String getSqlTypeName(Integer code, String length) {
		if(code == Types.VARCHAR && !StringUtil.isBlank(length)){
	...
	public void createTableAsSelect(Table table, Table changedTable, boolean dropFirst) {
		...
		_writer.append(" AS SELECT "+getColumnNameString(changedTable)+" FROM " + changedTable.getName());
	...
	private String getColumnString(Table table) {
		...
		if (columns != null && !columns.isEmpty()) {
			for (Iterator<Column> iter = columns.iterator(); iter.hasNext();) {
				...
				buffer.append(column.getName());
				buffer.append(" " + getSqlTypeName(Integer.valueOf(code), column.getLength()));
	...
	public void addColumn(Table changedTable, Column targetColumn) {
		...
		_writer.append("add " + colName + " " + getSqlTypeName(typeCode, targetColumn.getLength()));
		_writer.append(SQL_DELIMITER);
	}
	...
	public void modifyColumnDataType(Table changedTable, Column targetColumn) {
		...
		String targetTypeName = getSqlTypeName(Integer.valueOf(targetColumn.getTypeCode()), targetColumn.getLength());
