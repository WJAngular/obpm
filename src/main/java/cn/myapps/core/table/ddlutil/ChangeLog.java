...
public class ChangeLog {
	 ...
	 * @throws OBPMValidateException 
	 */
	private void compareFixedFieldsToAdd() throws DuplicateException, OBPMValidateException {
		Collection<Column> fixedColumns = getFixedColumns();
		for (Iterator<Column> iterator = fixedColumns.iterator(); iterator
				.hasNext();) {
			Column newCol = (Column) iterator.next();
	...
	private void compareFieldsToAddOrModify(Collection<FormField> newFields,
		...
		for (Iterator<FormField> iter = newFields.iterator(); iter.hasNext();) {
			FormField newfield = (FormField) iter.next();
			FormField oldfield = oldForm.findField(newfield.getId());
			if(oldfield == null){
				//字段id改变了,此时须根据字段名称获取FormField
				oldfield = oldForm.findFieldByName(newfield.getName());
			}
	...
	public void compareFieldsToDrop(Collection<FormField> oldFields,
			Form newForm) throws DuplicateException, OBPMValidateException {
		if (_newTable == null) {
			return;
		}

		for (Iterator<FormField> iter = oldFields.iterator(); iter.hasNext();) {
			FormField oldField = (FormField) iter.next();
	...
	private Collection<Column> getFixedColumns() {
		...
		Column authorDeptIndex = new Column("", "AUTHOR_DEPT_INDEX", Types.VARCHAR,"2000");
...
//		Column sortid = new Column("", "SORTID", Types.VARCHAR);
		...
		Column stateLableInfo = new Column("", "STATELABELINFO", Types.CLOB);
		Column prevAuditNode = new Column("", "PREVAUDITNODE", Types.CLOB);
		Column prevAuditUser = new Column("", "PREVAUDITUSER", Types.CLOB);
		Column optionItem = new Column("", "OPTIONITEM", Types.CLOB);
...
//		rtn.add(sortid);
		...
		rtn.add(domainid);
		rtn.add(auditorList);
		rtn.add(stateLableInfo);
		rtn.add(prevAuditNode);
		rtn.add(prevAuditUser);
		rtn.add(optionItem);
		return rtn;
	}
