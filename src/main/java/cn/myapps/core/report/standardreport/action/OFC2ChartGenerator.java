...
public class OFC2ChartGenerator {
	...
	public static String getBarChart(Collection<Map<String, String>> data, String[] cols){
		...
		while(iter.hasNext()) {
			...
			usedtime = Double.parseDouble((String)map.get("USEDTIME"));
			bar.addValues(new Number[]{usedtime});

			x_axis = x_axis.addLabels(new String[]{tempLables});
		...
		chart.addElements(new Element[]{bar});
	...
	public static String getLineChart(Collection<Map<String, String>> data, String[] cols){
		...
		chart.addElements(new Element[]{line});
		chart.setBackgroundColour("#dfe8f6");
		max = max +(int)(max*0.2);
		chart.setXAxis(x_axis);
	...
	public static String getPieChart(Collection<Map<String, String>> data, String[] cols){
		...
		while(iter.hasNext()) {
			...
			pie.addSlice(Double.parseDouble((String)map.get("USEDTIME")),tempLables);
			if(index<16){
