...
public class ConsoleRow {
		...
		if (metaDataSet != null && rowSet != null) {
			...
			if (metaDataSet != null) {
				for (Iterator<String> iter = set.iterator(); iter.hasNext();) {
					...
					ConsoleMetaData metaData = (ConsoleMetaData) metaDataSet.get(fieldName);
	...
	public ConsoleRow(long primaryKey,IRunner runner, Document doc,WebUser webUser,Set<Column> columns,Map<String , ConsoleMetaData> metaDataSet) throws Exception {
		...
		while (iter.hasNext()) {
			...
			String result = (String)col.getTextString(doc, runner, webUser);
			ConsoleMetaData metaData = (ConsoleMetaData) metaDataSet.get(col.getName());
	...
	public ConsoleData getData(ConsoleMetaData metaData) {
		return (ConsoleData) ((datas != null && metaData != null) ? datas.get(metaData
				.getColumnName()) : null);
	}
}
