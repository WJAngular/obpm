...
public class ConsoleData {
		...
		try{
			...
			if (metaData.getDataType().getValue() == ConsoleDataType.Boolean
				...
				if(value.indexOf(".")>0){
					...
					numericValue = (Double) f.parse(value);
				}else{
			...
			throw new Exception(metaData.getColumnName()+"{*[cn.myapps.core.report.crossreport.not_number_type]*}");
		}
	}
	...
	public ConsoleData(ConsoleMetaData metaData, String stringValue)
		...
		if (metaData.getDataType() == ConsoleDataType.Other)
			throw new OBPMValidateException("Unsupport Data Type");
	}
	...
	public String getStringValue() {
		if (metaData != null) {
			...
			if (metaData.getDataType() == ConsoleDataType.Numberic)
				return df1.format(numericValue);
			...
			if (metaData.getDataType() == ConsoleDataType.DateTime) {
				SimpleDateFormat format = new SimpleDateFormat(
						SystemParameters.SRV_DATETIME_FORMAT);
				return (dateValue != null) ? format.format(dateValue) : stringValue;
			}
		}
		return (stringValue != null) ? stringValue : "";
	}
