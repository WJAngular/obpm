...
//			String[] filters = (String[]) colls.toArray(new String[colls.size()]);
//			this.setSelectedAnalyseFilter(filters);
	...
	public String doSearchForm() throws Exception{
		...
		if(view.getSearchForm()!=null){
			...
			StringBuffer html = new StringBuffer();
			html.append("<span style='color:red'>").append(e.getMessage()).append("</span>");
			str = html.toString();
		}
		return SUCCESS;
	}
...
//			this.setSelectedAnalyseFilter(filters);
			...
			writer.write(xlsFile, analyseTable);
		} catch (Exception e) {
			addFieldError("", e.getMessage());
			e.printStackTrace();
	...
	public String doExport() throws Exception {
		...
		try {
			...
			String fileName = cn.myapps.util.sequence.Sequence.getSequence() + ".xls";
		...
		if (availableAnalyseRowSet == null) {
			if (datas != null) {
				...
				for (Iterator<ConsoleMetaData>  iterator = datas.getMetaDataSetIterator(); iterator.hasNext();) {
					ConsoleMetaData metaData = (ConsoleMetaData) iterator.next();
	 ...
	 * @return
	 */
	public String toSearchFormHtml(View view) throws Exception {
		try {
	...
	private Collection<ConsoleMetaData> prepareAnalyseColumnSet() {
		...
		if (selectedAnalyseColumnSet.iterator().hasNext()) {
			for (Iterator<Object> iterator = selectedAnalyseColumnSet.iterator(); iterator.hasNext();) {
				...
				ConsoleMetaData metaData = (ConsoleMetaData) metaDateSet.get(colName);
...
//			ConsoleMetaData metaData = (ConsoleMetaData) metaDateSet.get(selectedAnalyseFilter[i]);
...
//		}
	...
	private ConsoleMetaData prepareCalculationFieldSet() {
		Map<String, ConsoleMetaData>  metaDateSet = datas.getMetaDataSet();
		return (ConsoleMetaData) metaDateSet.get(selectedCalculationField);
	}
...
*/
	private String getTitleHtml(String title) throws Exception {
		...
		html += "{*[cn.myapps.core.report.crossreport.report]*}:" + (title == null ? "" : title);
		html += "</td>";
		html += "</tr>";
		html += "</table></div>";
		return html;
