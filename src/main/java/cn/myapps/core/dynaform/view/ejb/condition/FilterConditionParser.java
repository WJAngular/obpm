...
import cn.myapps.core.department.action.DepartmentHelper;
...
//						visitor.visitDateValue(params.getParameterAsString(match), dateField.getDatePattern());
					...
					} else if (Item.VALUE_TYPE_TEXT.equals(fieldType)) {
						visitor.visitTextValue(params.getParameterAsString(match));
					} else {
						// 可设置字符类型
						visitor.visitStringValue(params.getParameterAsString(match));
					}
