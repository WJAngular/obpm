...
public class XLSDocumentImprotProvider extends AbstractImportProvider
	...
	public int getMasterSheetRowCount() throws Exception {
		...
		if (sheet == null) {
			throw new OBPMValidateException(ms.name + " sheet table doesn't exist in your Excel file!");
		}
		...
		for (short row = 1; row < sheet.getLastRowNum(); row++) {
			...
			for (int i = 0; i < sheetHeads.getLastCellNum(); i++) {
				if (sheetHeads.getCell(i) != null && getCellStringValue(sheetHeads.getCell(i)) != null) {
					...
					if (!columnName.equalsIgnoreCase(getCellStringValue(sheetHeads.getCell(i))))
						continue;

					if (matchValue != null && matchValue.equals(getCellStringValue(rowCells.getCell(i)))) {

						for (int k = 0; k < sheetHeads.getLastCellNum(); k++) {
							String content = (String) map.get(getCellStringValue(sheetHeads.getCell(k)));
			...
			try {
				...
				for (int i = 0; i < sheetHeads.getLastCellNum(); i++) {
					try {
						...
						e.printStackTrace();
						throw new OBPMValidateException("第 " + (i + 1) + " 列->" + e.getMessage());
					}
				}

			} catch (Throwable e) {
				throw new OBPMValidateException(sheetName + " 第 " + (row + 1) + " 行->" + e.getMessage());
			}
