...
public class SaveNew extends ActivityType{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 7138563484034274646L;

	public SaveNew(Activity act) {
		super(act);
	}

	public String getOnClickFunction() {
		//return "doSave('" + act.getType() + "', '" + act.getId() + "')";
		return "Activity.doExecute('"+act.getId()+"',"+act.getType()+",{withOld:"+act.isWithOld()+"})";
	}
	...
	public String doExecute(AbstractRunTimeAction action, Document doc,
		...
		try {
			if (doSave(action, doc, user, params).equals(action.SUCCESS)) {
				/**
				if(act.isWithOld()){
	...
	public ValueObject doMbExecte(WebUser user, ParamsTable params)
		...
		try{
			...
			if(act.isWithOld()){
				...
				newDoc = proxy.doNewWithOutItems(form, user, params);
				MemoryCacheUtil.putToPrivateSpace(newDoc.getId(), newDoc, user);
			}
			return newDoc;
		}catch(Exception e){
