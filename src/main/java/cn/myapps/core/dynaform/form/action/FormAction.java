...
public class FormAction<E> extends BaseAction<E> {
	...
	static {
		_OPENTYPE.put(Integer.valueOf(View.OPEN_TYPE_NORMAL), "{*[NormalForm]*}");
	...
	static {
		_FORMTYPE.put(Integer.valueOf(Form.FORM_TYPE_NORMAL), "{*[NormalForm]*}");
		...
		"{*[TEMPLATEFORM]*}");
		_FORMTYPE.put(Integer.valueOf(Form.FORM_TYPE_FRAGMENT),
				"{*[cn.myapps.core.dynaform.form.type.fragment]*}");
	 ...
	 */
	public Map<Integer, String> get_FORMTYPE() {
		return _FORMTYPE;
	}
	 ...
	 * @see cn.myapps.base.action.BaseAction#doList()
	...
	public String doFormList() throws Exception {
		...
		//设置默认根据orderno排序
		String orderby = getParams().getParameterAsString("_orderby");
		...
		if(orderby == null || orderby.trim().length() <= 0){
			getParams().setParameter("_orderby", "orderno");
		}
		return super.doList();
	}
	...
	public String doClearData() {
		try {
			...
			if (null != deleteType) {
				((FormProcess) process).doClearFormData(form);
			} else {
				((FormProcess) process).doClearColumnData(form, fields);
			}
			this.addActionMessage("{*[clear.success]*}");
		}catch (OBPMValidateException e) {
			this.addFieldError("clear failed", "{*[clear.failed]*}"+e.getValidateMessage());
	...
	public String doOnekeycreview() {
		try {
			Form form = (Form) ((FormProcess) process)
	 ...
	 * 
	 * @see cn.myapps.base.action.BaseAction#doSave()
	...
	public String doSave() {
		try {
			...
			if (e.getCause() instanceof NeedConfirmException) {
				...
				this.addFieldError("[{*[Errors]*}]:  ", e.getValidateMessage());
			}
			return INPUT;
		}catch (Exception e) {
			LOG.error("doSave", e);
	...
	private void doSaveValidate(Form form) throws Exception {
		...
		if (invalidChars != null && !invalidChars.equals("")) {
			if(invalidChars.equals(" ")){
				throw new OBPMValidateException("{*[core.form.name.exist.invalidchar]*}: "
						+ "空格");
			}else{
				throw new OBPMValidateException("{*[core.form.name.exist.invalidchar]*}: "
						+ invalidChars);
		...
		if (!StringUtil.isBlank(relationName)
				&& !((FormProcess) process).checkRelationName(form.getId(),
						relationName)) {
			throw new OBPMValidateException(" {*[core.form.relationname.exist]*}: ");
		}
		
		//字段命名合法校验
		String invalidFieldName = getInvalidFieldName(form);
		if (!StringUtil.isBlank(invalidFieldName)){
			throw new OBPMValidateException("{*[FieldName]*} ["+invalidFieldName+" ]{*[core.form.fieldname.invalid]*}");
		}
...
//				addFieldError("", "{*[core.dynaform.form.message.warning.be.checkedout.by.other.developers]*}");
			...
			if(params.get("orderno") != null){
				String orderno = ((String[]) params.get("orderno"))[0];
				form.setOrderno(Integer.parseInt(orderno));
			}
			setContent(form);
		} catch (OBPMValidateException e) {
	...
	private String getInvalidFieldName(Form form){
		
		String[] sysFieldName = {"view_id","id","docid","_docid","viewid","_viewid","formid","_formid","_backurl","_selects","application","applicationid",
				"domain","domainid"};
		...
		for (Iterator<FormField> iterator = formFields.iterator(); iterator.hasNext();) {
			...
			for (int i = 0; i < sysFieldName.length; i++) {
				if (name != null && name.equalsIgnoreCase(sysFieldName[i])) {
					return sysFieldName[i];
				}
			}
