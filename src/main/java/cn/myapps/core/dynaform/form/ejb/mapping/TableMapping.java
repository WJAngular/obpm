package cn.myapps.core.dynaform.form.ejb.mapping;
...
public class TableMapping implements Serializable {
	...
	public String getColumnName(String fieldName) {
		ColumnMapping columnMapping = (ColumnMapping) columnMappingMap.get(fieldName);
	...
	public String getLogMappingFormColumnListString() {
		StringBuffer buffer = new StringBuffer();
		if (columnMappingMap != null && !columnMappingMap.isEmpty()) {
			Set<?> entrySet = columnMappingMap.entrySet();
			for (Iterator<?> iterator = entrySet.iterator(); iterator.hasNext();) {
				...
				ColumnMapping columnMapping = (ColumnMapping) entry.getValue();
				if (!columnMapping.isPrimaryKey()) {
					buffer.append("item_"+columnMapping.getFieldName());
					buffer.append(",");
				}
			}
