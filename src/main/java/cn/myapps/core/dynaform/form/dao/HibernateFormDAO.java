...
public class HibernateFormDAO<E> extends HibernateBaseDAO<E> implements FormDAO<E> {
	 ...
	 * @param voClassName
	 *            值对象类名
	 * @see cn.myapps.base.dao.HibernateBaseDAO#HibernateBaseDAO(String)
	 */
	...
	public void updateDynaTable(Form vo, Connection conn, boolean isOverlaid)
		...
		String doc_stuc = "ID VARCHAR2(255 CHAR) NOT NULL PRIMARY KEY , PARENT VARCHAR2(255 CHAR), LASTMODIFIED TIMESTAMP (6), FORMNAME VARCHAR2(255 CHAR), OWNER VARCHAR2(255 CHAR), STATE VARCHAR2(255 CHAR), AUDITDATE TIMESTAMP (6), AUTHOR VARCHAR2(255 CHAR), CREATED TIMESTAMP (6), FORMID VARCHAR2(255 CHAR), ISTMP NUMBER(1,0), VERSIONS NUMBER(10,0), APPLICATIONID VARCHAR2(255 CHAR), STATEINT NUMBER(10,0), STATELABEL VARCHAR2(255 CHAR),";
	...
	public Collection<E> queryFragmentFormsByModule(String moduleid,String application)
			throws Exception {
		String hql = "FROM " + _voClazzName + " vo WHERE vo.type=" + Form.FORM_TYPE_FRAGMENT;
		if(!StringUtil.isBlank(moduleid)){
			hql += " and vo.module.id='"+moduleid+"'";
		}
		ParamsTable params = new ParamsTable();
	...
	public Collection<Form> getFormsByApplication(String applicationId) throws Exception{
		String hql = "FROM " + _voClazzName + " vo WHERE vo.applicationid ='"+applicationId+"' order by vo.module.id desc";
		
		return (Collection<Form>) getDatas(hql);
	}
